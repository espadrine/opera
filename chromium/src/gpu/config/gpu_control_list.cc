...
GpuControlList::GpuControlListEntry::GetEntryFromValue(
  ...
  const base::ListValue* opera_bugs;
  if (value->GetList("opera_bugs", &opera_bugs)) {
    for (size_t i = 0; i < opera_bugs->GetSize(); ++i) {
      std::string bug_id;
      if (opera_bugs->GetString(i, &bug_id)) {
        entry->opera_bugs_.push_back(bug_id);
      } else {
        LOG(WARNING) << "Malformed opera_bugs entry " << entry->id();
        return NULL;
      }
    }
    dictionary_entry_count++;
...
void GpuControlList::GetReasons(base::ListValue* problem_list) const {
  ...
  for (size_t i = 0; i < active_entries_.size(); ++i) {
    ...
    base::ListValue* opera_bugs = new base::ListValue();
    for (size_t j = 0; j < entry->opera_bugs().size(); ++j)
      opera_bugs->Append(base::Value::CreateStringValue(entry->opera_bugs()[j]));
    problem->Set("operaBugs", opera_bugs);
