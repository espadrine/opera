...
bool GLES2DecoderImpl::Initialize(
  ...
  // TODO(jiangj): when offscreen=0 there will be no offscreen buffer created
  // (GetBackbufferServiceId() returns 0) so the DoBindFramebuffer() call above
  // will unbind current framebuffer. In this case, the glClear() call below
  // will become invalid operation. It's still unclear why upstream (ccameron)
  // insisted on doing it but we should keep this patch here first.
#if defined(OS_MACOSX)
  if (GetBackbufferServiceId())
  ...
  if (feature_info_->workarounds().
      restore_state_on_virtual_context_surface_change) {
    context_->ForceVirtualRestoreState();
    if (context_->share_group() &&
        context_->share_group()->GetSharedContext()) {
      context_->share_group()->GetSharedContext()->ForceVirtualRestoreState();
    }
  }
