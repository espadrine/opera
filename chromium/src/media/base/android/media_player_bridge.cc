...
void MediaPlayerBridge::SetVideoSurfaceHolder(jobject surface_holder) {
  if (j_media_player_.is_null() && surface_holder != NULL)
    Prepare();

  JNIEnv* env = base::android::AttachCurrentThread();
  CHECK(env);

  JNI_MediaPlayer::Java_MediaPlayer_setDisplay(
      env, j_media_player_.obj(), surface_holder);
}
