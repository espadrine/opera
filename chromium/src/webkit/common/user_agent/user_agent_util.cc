...
std::string BuildUserAgentFromProduct(const std::string& product1, const std::string product2) {
  const char kUserAgentPlatform[] =
      ...
      "";  // "Linux" not needed here as it is usually part of the OS CPU info
#else
      "Unknown; ";
#endif
  ...
  return BuildUserAgentFromOSAndProduct(os_info, product1, product2);
}
