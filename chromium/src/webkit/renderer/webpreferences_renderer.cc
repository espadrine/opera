...
void ApplyWebPreferences(const WebPreferences& prefs, WebView* web_view) {
  ...
  settings->setContextMenuOnMouseUp(prefs.context_menu_on_mouse_up);
  ...
  settings->setMediaBlockingEnabled(prefs.media_blocking_enabled);
  ...
  settings->setDefaultBackgroundColor(prefs.default_background_color);
  settings->setDefaultTextColor(prefs.default_text_color);
  ...
  settings->setTextLayoutStrategy(
      static_cast<WebSettings::TextLayoutStrategy>(prefs.text_layout_strategy));
