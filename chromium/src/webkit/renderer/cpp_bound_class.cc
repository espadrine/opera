...
class GetterIndexOperatorCallback
    : public CppBoundClass::IndexOperatorCallback {
 public:
  GetterIndexOperatorCallback(
      const CppBoundClass::IndexGetterCallback& callback)
      : callback_(callback) { }

  virtual bool GetValue(int32_t index, CppVariant *value) OVERRIDE {
    callback_.Run(index, value);
    return true;
  }

  virtual bool SetValue(int32_t index, const CppVariant &value) OVERRIDE {
    return false;
  }

 private:
  CppBoundClass::IndexGetterCallback callback_;
};
...
bool CppBoundClass::GetProperty(NPIdentifier ident, NPVariant* result) const {
  ...
  if (callback == properties_.end()) {
    if (index_callback_ && !WebBindings::identifierIsString(ident)) {
       int32_t index = WebBindings::intFromIdentifier(ident);
       CppVariant cpp_value;
       if (!index_callback_->GetValue(index, &cpp_value)) {
         VOID_TO_NPVARIANT(*result);
         return false;
       }
...
bool CppBoundClass::SetProperty(NPIdentifier ident,
  ...
  if (callback == properties_.end()) {
    if (index_callback_ && !WebBindings::identifierIsString(ident)) {
      ...
      CppVariant cpp_value;
      cpp_value.Set(*value);
      return index_callback_->SetValue(index, cpp_value);
    }
    return false;
  }
...
void CppBoundClass::BindIndexGetterCallback(
    const IndexGetterCallback& callback) {
  if (callback.is_null())
    index_callback_.reset();
  else
    index_callback_.reset(new GetterIndexOperatorCallback(callback));
}

void CppBoundClass::BindIndexOperator(IndexOperatorCallback *callback) {
  index_callback_.reset(callback);
}
...
CppVariant CppBoundClass::WrapInOwningCppVariant() {
  CppVariant owning_variant = *GetAsCppVariant();
  self_variant_.SetNull();

  NPObject* np_obj = NPVARIANT_TO_OBJECT(owning_variant);
  reinterpret_cast<CppNPObject*>(np_obj)->owns = true;
  return owning_variant;
}
...
CppBoundClass* CppBoundClass::GetForNPObject(NPObject* object) {
  if (!object || object->_class != &CppNPObject::np_class_)
    return NULL;

  CppNPObject* obj = reinterpret_cast<CppNPObject*>(object);
  return obj->bound_class;
}
