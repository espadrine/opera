...
const char* NPN_UserAgent(NPP id) {
  ...
  if (id) {
    scoped_refptr<PluginInstance> plugin = FindInstance(id);
    if (plugin.get()) {
      if (!plugin->use_mozilla_user_agent()) {
        use_mozilla_user_agent = false;

        // This is a nasty workaround for DNA-6044.
        // The ESN Launch plugin have internal mime type string buffer
        // that cannot accept more than 128 bytes. Unfortunatelly, plugin
        // does not check for the lenght of copied string and happily
        // overwrites memory when UA string is longer than the destination
        // buffer. This is dangerous and potentialy exploitable as only by
        // manipulating UA string one can overwite plugin instance pointer.
        // Since we don't know if other plugins from mozilla-plugin-esn-* plugin
        // family are affected or not we report short UA string for all
        // mozilla-plugin-esn-* plugins.
        if (StartsWithASCII(
            plugin->mime_type(), "application/mozilla-plugin-esn", false)) {
          return "Mozilla/5.0 (Windows NT 6.1; WOW64) "
                 "AppleWebKit/537.28 (KHTML, like Gecko) "
                 "Chrome/26.0.1395.0 Safari/537.28";
        }
      }
    }
  }
