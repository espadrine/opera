...
// static
FundamentalValue* Value::CreateInt64Value(int64 in_value) {
  return new FundamentalValue(in_value);
}
...
bool Value::GetAsPickle(PickleValue** out_value) {
  return false;
}

bool Value::GetAsPickle(const PickleValue** out_value) const {
  return false;
}
...
FundamentalValue::FundamentalValue(int64 in_value)
    : Value(TYPE_INT64), int64_value_(in_value) {
}
...
bool FundamentalValue::GetAsInt64(int64* out_value) const {
  if (out_value && IsType(TYPE_INT64))
    *out_value = int64_value_;
  else if (out_value && IsType(TYPE_INTEGER))
    *out_value = integer_value_;
  return (IsType(TYPE_INT64) || IsType(TYPE_INTEGER));
}
...
bool FundamentalValue::GetAsDouble(double* out_value) const {
  if (out_value && IsType(TYPE_DOUBLE))
    ...
    *out_value = integer_value_;
  else if (out_value && IsType(TYPE_INT64))
    *out_value = int64_value_;
  return (IsType(TYPE_DOUBLE) || IsType(TYPE_INTEGER) || IsType(TYPE_INT64));
}
...
FundamentalValue* FundamentalValue::DeepCopy() const {
  switch (GetType()) {
    ...
    case TYPE_INT64:
      return CreateInt64Value(int64_value_);
...
bool FundamentalValue::Equals(const Value* other) const {
  ...
  switch (GetType()) {
    ...
    case TYPE_INT64: {
      int64 lhs, rhs;
      return GetAsInt64(&lhs) && other->GetAsInt64(&rhs) && lhs == rhs;
    }
    case TYPE_DOUBLE: {
      double lhs, rhs;
...
void DictionaryValue::SetInt64(const std::string& path, int64 in_value) {
  Set(path, CreateInt64Value(in_value));
}
...
void DictionaryValue::SetInt64WithoutPathExpansion(
    const std::string& path, int64 in_value) {
  SetWithoutPathExpansion(path, CreateInt64Value(in_value));
}
...
bool DictionaryValue::GetPickle(const std::string& path,
                                const PickleValue** out_value) const {
  const Value* value;
  bool result = Get(path, &value);
  if (!result || !value->IsType(TYPE_PICKLE))
    return false;

  if (out_value)
    *out_value = static_cast<const PickleValue*>(value);

  return true;
}

bool DictionaryValue::GetPickle(const std::string& path,
                                PickleValue** out_value) {
  return static_cast<const DictionaryValue&>(*this).GetPickle(
      path,
      const_cast<const PickleValue**>(out_value));
}
...
bool DictionaryValue::GetPickleWithoutPathExpansion(
    const std::string& key,
    const PickleValue** out_value) const {
  const Value* value;
  bool result = GetWithoutPathExpansion(key, &value);
...
bool DictionaryValue::GetPickleWithoutPathExpansion(const std::string& key,
                                                    PickleValue** out_value) {
  return
      static_cast<const DictionaryValue&>(*this).GetPickleWithoutPathExpansion(
          key,
          const_cast<const PickleValue**>(out_value));
}
...
void ListValue::AppendInt64(int64 in_value) {
  Append(CreateInt64Value(in_value));
}
...
PickleValue::PickleValue() : Value(TYPE_PICKLE) {
}

PickleValue::PickleValue(const char* data, int data_len)
    : Value(TYPE_PICKLE),
      pickle_(data, data_len) {
  copied_pickle_data_.reset(data);
}

PickleValue::~PickleValue() {
}

PickleValue* PickleValue::CreateWithCopiedBuffer(const char* data,
                                                 int data_len) {
  char* pickle_data = new char[data_len];
  memcpy(pickle_data, data, data_len);

  return new PickleValue(pickle_data, data_len);
}

bool PickleValue::GetAsPickle(PickleValue** out_value) {
  if (out_value)
    *out_value = this;
  return true;
}

bool PickleValue::GetAsPickle(const PickleValue** out_value) const {
  if (out_value)
    *out_value = this;
  return true;
}

Value* PickleValue::DeepCopy() const {
  PickleValue* copy = new PickleValue();
  copy->pickle_ = pickle_;
  return copy;
}
