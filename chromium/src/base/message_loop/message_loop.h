...
#if !defined(OS_MACOSX) && !defined(OS_ANDROID) && !defined(LINUX_EMBEDDED)
...
#if defined(OS_ANDROID) || defined(LINUX_EMBEDDED)
...
class BASE_EXPORT MessageLoop : public MessagePump::Delegate {
 public:
  ...
  typedef void (*MemoryAllowanceCB)();
  ...
  // Wake up the message processing to do a memory allowance check.
  void MemoryAllowanceWakeUp();

  // Sets the memory allowance callback.
  // The message loop calls the callback after each task to
  // trigger a memory allowance check.
  void SetMemoryAllowanceCB(MemoryAllowanceCB cb);
 ...
 private:
  ...
  MemoryAllowanceCB memory_allowance_cb_;
...
class BASE_EXPORT MessageLoopForUI : public MessageLoop {
 public:
  ...
  void Start();
#elif !defined(OS_MACOSX) && !defined(LINUX_EMBEDDED)
