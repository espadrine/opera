...
void Layer::SetExternalBitmap(const SkBitmap* bitmap) {
  DCHECK_EQ(type_, LAYER_TEXTURED);
  DCHECK(!solid_color_layer_);
  bool has_bitmap = !!bitmap;
  layer_updated_externally_ = has_bitmap;
  if (!!image_layer_ != has_bitmap) {
    if (has_bitmap) {
      scoped_refptr<cc::ImageLayer> new_layer =
          cc::ImageLayer::Create();
      new_layer->SetIsMask(true);
      SwitchToLayer(new_layer);
      image_layer_ = new_layer;
    } else {
      scoped_refptr<cc::ContentLayer> new_layer =
          cc::ContentLayer::Create(this);
      SwitchToLayer(new_layer);
