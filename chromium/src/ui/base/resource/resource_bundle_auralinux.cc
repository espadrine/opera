...
void ResourceBundle::LoadCommonResources() {
  ...
  AddDataPackFromPath(GetResourcesPakFilePath(COMMON_RESOURCES_PREFIX ".pak"),
                      SCALE_FACTOR_NONE);
  AddDataPackFromPath(GetResourcesPakFilePath(
      COMMON_RESOURCES_PREFIX "_100_percent.pak"), SCALE_FACTOR_100P);

  if (ui::IsScaleFactorSupported(SCALE_FACTOR_200P)) {
    AddOptionalDataPackFromPath(GetResourcesPakFilePath(
        COMMON_RESOURCES_PREFIX "_200_percent.pak"), SCALE_FACTOR_200P);
  }
}
