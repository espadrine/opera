...
void ResourceBundle::LoadCommonResources() {
  ...
  switch(ui::GetScaleFactorFromScale(scale)) {
    case ui::SCALE_FACTOR_180P:
      AddDataPackFromPath(GetResourcesPakFilePath(
          COMMON_RESOURCES_PREFIX "_touch_180_percent.pak"),
          SCALE_FACTOR_180P);
      force_touch_resources = true;
      break;
    case ui::SCALE_FACTOR_140P:
      AddDataPackFromPath(GetResourcesPakFilePath(
          COMMON_RESOURCES_PREFIX "_touch_140_percent.pak"),
          SCALE_FACTOR_140P);
      force_touch_resources = true;
  }
  ...
  if (ui::GetDisplayLayout() == ui::LAYOUT_TOUCH || force_touch_resources) {
    AddDataPackFromPath(
        GetResourcesPakFilePath(COMMON_RESOURCES_PREFIX "_touch_100_percent.pak"),
        SCALE_FACTOR_100P);
  } else {
    AddDataPackFromPath(
        GetResourcesPakFilePath(COMMON_RESOURCES_PREFIX "_100_percent.pak"),
        SCALE_FACTOR_100P);
  }
}
