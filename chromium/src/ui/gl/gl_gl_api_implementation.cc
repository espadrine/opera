...
bool VirtualGLApi::MakeCurrent(GLContext* virtual_context, GLSurface* surface) {
  ...
  bool force_restore_state = false;
  ...
  if (switched_contexts || surface != current_surface) {
    ...
    if (switched_contexts || !current_surface ||
      ...
      // TODO(tjarosik): Only glScissor() is needed to restore for qualcomm,
      // and there is already "workarounds().restore_state_on_surface_change".
      // I don't know what scissor rect to set here, so restoring a whole
      // state.
      force_restore_state = force_restore_state_;
...
void VirtualGLApi::SetForceRestoreState() {
  force_restore_state_ = true;
}
