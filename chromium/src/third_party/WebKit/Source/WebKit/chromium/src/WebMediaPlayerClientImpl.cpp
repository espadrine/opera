...
void WebMediaPlayerClientImpl::prepareToPlay()
{
    if (m_delayingLoad)
        startDelayedLoad();
    else if (m_webMediaPlayer)
      return m_webMediaPlayer->prepareToPlay();
}
...
// static
WebKit::WebMimeRegistry::SupportsType MediaPlayer::supportsType(
    const String& type,
    const String& codecs,
    const String& keySystem) {
  return WebKit::WebMediaPlayer::supportsType(type, codecs, keySystem);
}
}
#endif
