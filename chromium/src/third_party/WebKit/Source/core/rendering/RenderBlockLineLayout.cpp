...
public:
    ...
    void updateAvailableWidth(LayoutUnit minimumHeight = 0, bool limitForTextWrap = false);
    ...
    void fitBelowFloats(bool limitForTextWrap = false);
...
inline void LineWidth::updateAvailableWidth(LayoutUnit replacedHeight, bool limitForTextWrap)
{
...
void LineWidth::fitBelowFloats(bool limitForTextWrap)
{
...
InlineIterator RenderBlock::LineBreaker::nextSegmentBreak(InlineBidiResolver& resolver, LineInfo& lineInfo, RenderTextInfo& renderTextInfo, FloatingObject* lastFloatFromPreviousLine, unsigned consecutiveHyphenatedLines, WordMeasurements& wordMeasurements)
{
    ...
    LayoutUnit replacedBoxHeight = 0;
    ...
    while (current.m_obj) {
        ...
        if (current.m_obj->isOutOfFlowPositioned()) {
            ...
            if (atStart) {
                replacedBoxHeight = replacedBox->logicalHeight();
                width.updateAvailableWidth(replacedBoxHeight);
            }
            ...
            bool limitLineWidthForTextWrap = blockHasTextToWrap;

#if ENABLE(TEXT_WRAPPING)
            // Don't wrap text in certain kind of containers.
            RenderObject* container = current.m_obj->container();
            while (limitLineWidthForTextWrap && container) {
                if (container->isTextField() || container->isTextArea() || container->isMenuList())
                    limitLineWidthForTextWrap = false;
                container = container->container();
            }

            width.updateAvailableWidth(replacedBoxHeight, limitLineWidthForTextWrap);
#endif // ENABLE(TEXT_WRAPPING)
            ...
            for (; current.m_pos < t->textLength(); current.fastIncrementInTextNode()) {
                ...
                if (betweenWords || midWordBreak) {
                    ...
                    if (!width.committedWidth() && autoWrap && !width.fitsOnLine())
                        width.fitBelowFloats(limitLineWidthForTextWrap);
