...
void RenderBlock::limitForTextWrap(float& outWidth)
{
    const Settings* settings = document()->settings();
    const FrameView* frameView = view()->frameView();
    if (frameView && settings && settings->textLayoutStrategy() == TextLayoutWrapping) {
        const int pixelPadding = 15;
        const float padding = pixelPadding * frameView->frame()->page()->deviceScaleFactor();
        const int textWrapWidth = frameView->textWrapWidth();
        const float maxWidth = textWrapWidth - padding;
        if (maxWidth > 0)
            outWidth = min(outWidth, maxWidth);
    }
}
