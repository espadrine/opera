...
Chrome::Chrome(Page* page, ChromeClient* client)
    : m_page(page)
    , m_client(client)
    , m_playStateTimer(this, &Chrome::playStateTimerFired)
    , m_playState(MediaInactive)
{
    ASSERT(m_client);
}
...
void Chrome::textLayoutStrategyChanged() const
{
    m_client->textLayoutStrategyChanged();
}
...
bool Chrome::clickToPlayPauseVideo()
{
    return m_client->clickToPlayPauseVideo();
}
...
void Chrome::invalidatePlayState()
{
    if (!m_playStateTimer.isActive())
        m_playStateTimer.startOneShot(0);
}

void Chrome::playStateTimerFired(Timer<Chrome>*)
{
    // Based on Document::hasPlayingMedia and ::hasSuspendedMedia for all the
    // documents in the page, calculate the page-wide play state. When there are
    // both playing and suspended media elements, clear the suspend state of all
    // documents. This is done so that a user manually playing a media element
    // will forget about the old state and not later try to resume playback of
    // prior suspended elements.

    bool hasPlayingMedia = false;
    bool hasSuspendedMedia = false;

    for (Frame* frame = m_page->mainFrame(); frame; frame = frame->tree()->traverseNext()) {
        if (Document* document = frame->document()) {
            if (document->hasPlayingMedia())
                hasPlayingMedia = true;
            if (document->hasSuspendedMedia())
                hasSuspendedMedia = true;
        }
    }

    if (hasPlayingMedia && hasSuspendedMedia) {
        for (Frame* frame = m_page->mainFrame(); frame; frame = frame->tree()->traverseNext()) {
            if (Document* document = frame->document())
                document->clearSuspendedMedia();
        }
        hasSuspendedMedia = false;
    }
    ...
    if (hasPlayingMedia)
        playState = MediaPlaying;
    else if (hasSuspendedMedia)
        playState = MediaSuspended;
    else
        playState = MediaInactive;

    if (m_playState != playState) {
        m_playState = playState;
        m_client->playStateChanged(playState);
    }
}
