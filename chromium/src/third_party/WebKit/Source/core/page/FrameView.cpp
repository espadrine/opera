...
void FrameView::setFrameRect(const IntRect& newRect)
{
    ...
    if (newRect.width() != oldRect.width()) {
        ...
        if (page && page->mainFrame() == m_frame && page->settings()->textLayoutStrategy() == TextLayoutAutosizing) {
...
void FrameView::textLayoutStrategyChanged() const
{
    Page* page = frame() ? frame()->page() : NULL;
    if (page)
        page->chrome().textLayoutStrategyChanged();
}
