...
public:
    ...
    static PassRefPtr<Document> createCEHTML(Frame* frame, const KURL& url)
    {
        return adoptRef(new Document(frame, url, XHTMLDocumentClass | CEHTMLDocumentClass));
    }
    virtual ~Document();
    ...
    // HbbTV documents are also considered CE-HTML documents.
    bool isCEHTMLDocument() const { return m_documentClasses & CEHTMLDocumentClass; }
    ...
    */
    ...
    // Suspend and resume media element playback, to support global temporary pausing.
    void suspendMedia();
    void resumeSuspendedMedia();
    void clearSuspendedMedia();
    bool hasPlayingMedia();
    bool hasSuspendedMedia();
