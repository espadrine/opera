...
bool HTMLDocument::moveFocusInDirection(FocusDirection direction) {
  if (!page() || !page()->settings() || !page()->settings()->spatialNavigationEnabled())
      return false;

  if (page()->focusController()->handleCSSFocusNavigation(direction))
      return true;

  if (page()->focusController()->advanceFocus(direction, NULL))
      return true;
