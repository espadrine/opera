...
bool SelectorChecker::checkOne(const SelectorCheckingContext& context, const SiblingTraversalStrategy& siblingTraversalStrategy) const
{
    ...
    if (selector->m_match == CSSSelector::PseudoClass) {
        ...
        case CSSSelector::PseudoRequiresFullscreen:
#if OS(ANDROID)
            if (element && element->isMediaElement() && element->document() && element->document()->page()) {
                HTMLMediaElement* mediaElement = static_cast<HTMLMediaElement*>(element);
                return mediaElement->isVideo() && element->document()->page()->chrome().requiresFullscreenForVideoPlayback();
            }
#endif
            return false;
