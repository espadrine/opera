...
bool ZipReader::ExtractCurrentEntryIntoStream(std::stringstream* stream) {
  DCHECK(zip_file_);
...
int ZipReader::ExtractCurrentEntryIntoMemory(unsigned char* buffer,
                                             int buffer_size) {
  DCHECK(zip_file_);
  ...
  while (true) {
    ...
    if (num_bytes_read == 0) {
      ...
      if (copied + num_bytes_read > buffer_size) {
        success = false;
        break;
      }
      memcpy(buffer + copied, buf, num_bytes_read);
      copied += num_bytes_read;
    }
  }

  return success ? copied : -1;
}
