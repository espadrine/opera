...
class ZipReader {
 public:
  ...
  // Extracts the current entry to the given stream.
  //
  // Returns false if eror ocuured.
  bool ExtractCurrentEntryIntoStream(std::stringstream* stream);

  // Extracts the current entry to the given memory buffer
  //
  // Returns the number of copied bytes. -1 is returned if eror ocuured.
  int ExtractCurrentEntryIntoMemory(unsigned char* buffer, int buf_size);
