...
NetPrefObserver::NetPrefObserver(PrefService* prefs,
  ...
  turbo_enabled_.Init(prefs::kTurboEnabled, prefs, prefs_callback);
  ...
  proxy_for_local_servers_enabled_.Init(prefs::kProxyForLocalServers, prefs, prefs_callback);
...
void NetPrefObserver::ApplySettings() {
  ...
  net::HttpStreamFactory::set_turbo_enabled(*turbo_enabled_);
  if (spdy_disabled_.IsManaged())
    net::HttpStreamFactory::set_spdy_enabled(!*spdy_disabled_);
  net::HttpStreamFactory::set_proxy_for_local_servers_enabled(*proxy_for_local_servers_enabled_);
}
...
void NetPrefObserver::RegisterUserPrefs(
  ...
  registry->RegisterStringPref(
      prefs::kTurboClientID,
      "",
      user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF);
  registry->RegisterDictionaryPref(
      prefs::kTurboUrlBlacklist,
      user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF);
  registry->RegisterBooleanPref(
      prefs::kDisableSpdy,
      false,
