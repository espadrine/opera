...
int ChromeNetworkDelegate::OnBeforeURLRequest(
  ...
  if (net::HttpStreamFactory::turbo_enabled() &&
      turbo_blacklist_manager_ &&
      turbo_blacklist_manager_->IsURLBlocked(request->url())) {
    int l = request->load_flags();
    request->set_load_flags(l | net::LOAD_BYPASS_TURBO);
  }
#endif
...
#if defined(ENABLE_CONFIGURATION_POLICY)
  if (net::HttpStreamFactory::turbo_enabled() &&
      original_response_headers->HasHeader("retry-after") &&
      original_response_headers->response_code() == 503) {
    std::string value;
    base::Time expiry_time;
    if (original_response_headers->EnumerateHeader(NULL, "retry-after", &value)) {
      int64 expiry;
    ...
    if (original_response_headers->EnumerateHeader(&iter, "x-turbo-bypass",
      ...
      if (turbo_blacklist_manager_) {
        turbo_blacklist_manager_->Block("path=\"/\"; domain=\"" +
                                        request->url().host() + "\";",
                                        expiry_time);
      }
    }
