...
TabSpecificContentSettings::TabSpecificContentSettings(WebContents* tab)
  ...
  for (size_t i = 0; i < TURBO_CONTENT_SETTINGS_NUM_TYPES; ++i)
    turbo_content_blocked_resources_[i] = 0;
...
bool TabSpecificContentSettings::IsContentBlockedByTurbo(
    TurboContentSettingsType content_type) const {
  return turbo_content_blocked_resources_[content_type] != 0;
}
...
void TabSpecificContentSettings::OnTurboContentBlocked(
    TurboContentSettingsType type,
    const std::string& resource_identifier) {
  ++turbo_content_blocked_resources_[type];
  if (turbo_content_blocked_resources_[type] == 1) {
    content::NotificationService::current()->Notify(
        chrome::NOTIFICATION_WEB_CONTENT_SETTINGS_CHANGED,
        content::Source<WebContents>(web_contents()),
        content::NotificationService::NoDetails());
...
void TabSpecificContentSettings::OnBlockedTurboContentRemoved(
    TurboContentSettingsType type,
    const std::string& resource_identifier) {
  DCHECK_GT(turbo_content_blocked_resources_[type], 0);
  --turbo_content_blocked_resources_[type];
  if (turbo_content_blocked_resources_[type] == 0) {
    content::NotificationService::current()->Notify(
        chrome::NOTIFICATION_WEB_CONTENT_SETTINGS_CHANGED,
        content::Source<WebContents>(web_contents()),
        content::NotificationService::NoDetails());
...
bool TabSpecificContentSettings::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(TabSpecificContentSettings, message)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_TurboContentBlocked, OnTurboContentBlocked)
    IPC_MESSAGE_HANDLER(ViewHostMsg_BlockedTurboContentRemoved,
        OnBlockedTurboContentRemoved);
