...
void DownloadDatabase::QueryDownloads(
  ...
  sql::Statement statement_main(GetDB().GetCachedStatement(SQL_FROM_HERE,
      "SELECT id, current_path, target_path, mime_type, original_mime_type, "
...
bool DownloadDatabase::UpdateDownload(const DownloadRow& data) {
  ...
  sql::Statement statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
      "UPDATE downloads "
      "SET current_path=?, target_path=?, mime_type=?, original_mime_type=?, "
          "received_bytes=?, state=?, danger_type=?, interrupt_reason=?, "
          "end_time=?, total_bytes=?, opened=? WHERE id=?"));
  int column = 0;
...
int64 DownloadDatabase::CreateDownload(const DownloadRow& info) {
...

  {
    sql::Statement statement_insert(GetDB().GetCachedStatement(
        SQL_FROM_HERE,
        "INSERT INTO downloads "
        "(id, current_path, target_path, mime_type, original_mime_type, "
        " start_time, received_bytes, total_bytes, state, danger_type, "
        " interrupt_reason, end_time, opened, refferer) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"));
