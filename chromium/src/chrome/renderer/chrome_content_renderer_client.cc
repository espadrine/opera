    ...
    switch (status_value) {
      ...
      case ChromeViewHostMsg_GetPluginInfo_Status::kAllowed: {
        ...
        if (view && view->settings() &&
            view->settings()->mediaBlockingEnabled() &&
            !document.isPluginDocument()) {
          placeholder = PluginPlaceholder::CreateTurboBlockedPlugin(
              render_view, frame, params, plugin, identifier, group_name);
          render_view->TurboContentBlocked(
...
std::string ChromeContentRendererClient::GetTurboBlockedHtml() const {
