...
// static
PluginPlaceholder* PluginPlaceholder::CreateTurboBlockedPlugin(
    RenderView* render_view,
    WebFrame* frame,
    const WebPluginParams& params,
    const WebPluginInfo& plugin,
  ...
  values.SetString("long_message",
                   l10n_util::GetStringUTF16(IDS_TURBO_CONTENT_LOAD_LONG));
  values.SetString("short_message",
                   l10n_util::GetStringUTF16(IDS_TURBO_CONTENT_LOAD_SHORT));

  const base::StringPiece template_html(
      ResourceBundle::GetSharedInstance().GetRawDataResource(
          IDR_TURBO_BLOCKED_PLUGIN_HTML));

  DCHECK(!template_html.empty()) << "unable to load template. ID: "
                                 << IDR_TURBO_BLOCKED_PLUGIN_HTML;
  std::string html_data = webui::GetI18nTemplateHtml(
  ...
  blocked_plugin->plugin_info_ = plugin;
  blocked_plugin->identifier_ = identifier;
  blocked_plugin->is_blocked_by_turbo_ = true;
  return blocked_plugin;
}
...
void PluginPlaceholder::LoadBlockedByTurboContent() {
  if (is_blocked_by_turbo_)
    LoadPlugin();
}
...
void PluginPlaceholder::WillDestroyPlugin() {
  if (is_blocked_by_turbo_) {
    render_view()->BlockedTurboContentRemoved(
