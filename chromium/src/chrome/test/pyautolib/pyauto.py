...
class PyAutoTextTestRunner(unittest.TextTestRunner):
  ...
  def _makeResult(self):
    if self._stream:
      return pyauto_gtest_stream.GTestStreamProtocolTestResult(self.stream, self.descriptions, self.verbosity, self._stream)
    else:
...
class Main(object):
  ...
  def _ParseArgs(self):
    ...
    parser.add_option(
        '', '--stream-results', type='string', default=None,
        help='Stream the results using the gtest stream protocol to a specified host.')
  ...
  def _Run(self):
    ...
    if self._options.verbose:
      verbosity = 2
    result = PyAutoTextTestRunner(stream=_OPTIONS.stream_results, verbosity=verbosity).run(pyauto_suite)
