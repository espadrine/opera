...
HttpStreamFactoryImpl::Job::~Job() {
  ...
  if (out_req_)
    session_->params().host_resolver->CancelRequest(out_req_);
}
...
int HttpStreamFactoryImpl::Job::DoInternalIpCheck() {
  ...
  if (proxy_for_local_servers_enabled()) {
  ...
  origin_request_info_.reset(new HostResolver::RequestInfo(origin_));
  return session_->params().host_resolver->Resolve(*(origin_request_info_.get()),
						   &origin_addresses_, io_callback_, &out_req_, net_log_);
}
...
ProxyServer *HttpStreamFactoryImpl::Job::turbo_proxy_ = NULL;
HostPortPair *HttpStreamFactoryImpl::Job::turbo_host_port_pair_ = NULL;
...
int HttpStreamFactoryImpl::Job::DoInitConnection() {
  ...
  bool turbo_enabled = HttpStreamFactory::turbo_enabled() &&
                       !(request_info_.load_flags & LOAD_BYPASS_TURBO) &&
                       !(request_info_.load_flags & LOAD_IS_DOWNLOAD) &&
                       !(request_info_.load_flags & LOAD_BYPASS_PROXY);

  if (bypass_turbo_proxy_ || using_ssl_)
    turbo_enabled = false;
  ...
  if (turbo_enabled) {
    CheckTurboConfig();
    spdy_session_key = SpdySessionKey(origin_,
				      *turbo_proxy_,
				      kPrivacyModeDisabled);
  } else {
    spdy_session_key = GetSpdySessionKey();
  }
  ...
  if (IsPreconnecting()) {
    ...
    return PreconnectSocketsForHttpRequest(
        ...
        net_log_,
        num_streams_,
        turbo_enabled ? turbo_proxy_: NULL);
  } else {
...
int HttpStreamFactoryImpl::Job::DoCreateStream() {
  ...
  if (IsHttpsProxyAndHttpUrl()) {
    ...
    CheckTurboConfig();
    spdy_session_key = SpdySessionKey(*turbo_host_port_pair_,
				      *turbo_proxy_,
				      kPrivacyModeDisabled);
  }
...
void HttpStreamFactoryImpl::Job::CheckTurboConfig()
{
  if (!turbo_host_port_pair_) {
    turbo_host_port_pair_ = new net::HostPortPair("soyuz-turbo.opera-mini.net", 80);
  }
  if (!turbo_proxy_ && turbo_host_port_pair_) {
    turbo_proxy_ = new net::ProxyServer(net::ProxyServer::SCHEME_HTTP, *turbo_host_port_pair_);
  }
}
