...
class HttpStreamFactoryImpl::Job {
 ...
 private:
  ...
  // Check Turbo configuration setup
  void CheckTurboConfig();
  ...
  scoped_ptr<HostResolver::RequestInfo> origin_request_info_;
  ...
  HostResolver::RequestHandle out_req_;
  ...
  // We prevent turbo proxy from taking over during actual connection startup
  // if address is in private network range.
  bool bypass_turbo_proxy_;

  static ProxyServer *turbo_proxy_;
  static HostPortPair *turbo_host_port_pair_;
