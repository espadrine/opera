...
void SetResolveHostAsNonPrivateAddress(SessionDependencies* session_deps) {
  RuleBasedHostResolverProc* catchall = new RuleBasedHostResolverProc(NULL);
  catchall->AddIPLiteralRule("*", "52.3.0.1", "");
  session_deps->host_resolver->set_rules(catchall);
}
...
TEST(HttpStreamFactoryTest, PreconnectHttpProxy) {
  for (size_t i = 0; i < arraysize(kTests); ++i) {
    ...
    SetResolveHostAsNonPrivateAddress(&session_deps);
    scoped_refptr<HttpNetworkSession> session(CreateSession(&session_deps));
    HttpNetworkSessionPeer peer(session);
