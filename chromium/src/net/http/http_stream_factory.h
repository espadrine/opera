...
class NET_EXPORT HttpStreamFactory {
 public:
  ...
  static void set_turbo_enabled(bool value) {
    turbo_enabled_ = value;
  }
  static bool turbo_enabled() { return turbo_enabled_; }

  // Set turbo client ID.
  static void set_turbo_client_id(std::string value) {
    if (!turbo_client_id_)
      turbo_client_id_ = new std::string();
      *turbo_client_id_ = value;
  }
  static std::string get_turbo_client_id() {
    if (!turbo_client_id_)
      return std::string();
    else
