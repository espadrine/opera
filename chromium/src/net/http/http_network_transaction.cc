...
int HttpNetworkTransaction::DoCreateStreamComplete(int result) {
  ...
  turbo_enabled_ = HttpStreamFactory::turbo_enabled();
...
/* static */
std::string HttpNetworkTransaction::HexAsciiEncode(unsigned char* indata, unsigned int indata_size)
{
  std::string result;
...
std::string HttpNetworkTransaction::CreateTurboProxyAuth(int connection_id) {
  ...
  result = auth2;
  std::string hexAscii = HexAsciiEncode((unsigned char*)digest.c_str(), digest.length());
  result += hexAscii;
  return result;
}
...
bool HttpNetworkTransaction::turbo_proxy_auth_retry_ = false;
...
void HttpNetworkTransaction::BuildRequestHeaders(bool using_proxy) {
  ...
  bool turbo_enabled = turbo_enabled_ &&
                       !(request_->load_flags & LOAD_BYPASS_TURBO) &&
                       !(request_->load_flags & LOAD_IS_DOWNLOAD) &&
  ...
  if (turbo_enabled) {
    ...
    request_headers_.SetHeader(HttpRequestHeaders::kOperaInfo, base::StringPrintf("ID=%d, p=%d, f=%d, sw=1024, sh=768", request_->request_id, usage, features));
    ...
    if (!stream_->IsConnectionReused() || turbo_proxy_auth_retry_)
    {
      ...
      std::string turbo_client_id = HttpStreamFactory::get_turbo_client_id();
      request_headers_.SetHeader(HttpRequestHeaders::kOperaID, turbo_client_id);
  ...
  if (turbo_enabled) {
    request_headers_.SetHeader("Accept", "image/webp, */*");
  }
...
int HttpNetworkTransaction::DoReadHeadersComplete(int result) {
  ...
  if (HttpStreamFactory::turbo_enabled() &&
      response_.headers->HasHeader("retry-after") &&
  ...
  if (response_.headers->EnumerateHeader(NULL, HttpRequestHeaders::kOperaHost,
      &turbo_host_str)) {
