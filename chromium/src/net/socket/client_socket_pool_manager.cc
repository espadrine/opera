...
int InitSocketPoolHelper(const GURL& request_url,
  ...
  if (turbo_proxy)
    connection_group = "turboproxy";

  bool ignore_limits = (request_load_flags & LOAD_IGNORE_LIMITS) != 0;
  if (proxy_info.is_direct() && !turbo_proxy) {
	...
	ProxyServer proxy_server = turbo_proxy ? *turbo_proxy : proxy_info.proxy_server();
	proxy_host_port.reset(new HostPortPair(turbo_proxy ? turbo_proxy->host_port_pair() : proxy_server.host_port_pair()));
    ...
    if (proxy_info.is_http() || proxy_info.is_https() || turbo_proxy) {
...
int InitSocketHandleForHttpRequest(
  ...
  return InitSocketPoolHelper(
      ...
      0, socket_handle, resolution_callback, callback, turbo_proxy);
}
