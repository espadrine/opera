...
class AndroidNetworkLibrary {
    ...
    static public boolean storeKeyPair(Context context, byte[] public_key, byte[] private_key) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                intent = ICSCompatLayer.createInstallIntent();
            } else {
                intent = CompatKeyChain.createInstallIntent();
            }
        ...
        try {
            ...
            switch (cert_type) {
              ...
              case CertificateMimeType.X509_CA_CERT:
                intent.putExtra(CompatKeyChain.EXTRA_CERTIFICATE, data);
                break;

              case CertificateMimeType.PKCS12_ARCHIVE:
                intent.putExtra(CompatKeyChain.EXTRA_PKCS12, data);
                break;
    ...
    public static int verifyServerCertificates(byte[][] certChain, String authType) {
        try {
            ...
            Log.e(TAG, "Failed to verify server certificate: " + e.getMessage());
            return CertVerifyResultAndroid.VERIFY_FAILED;
        }
    }
    ...
    /**
     * Compatible class for KeyChain
     * See android source: base/keystore/java/android/security/KeyChain.java
     */
    private static class CompatKeyChain {
