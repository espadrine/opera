...
DEFAULT_EXTENSIONS_INCLUDE_PATH = 'chrome/browser/extensions'
...
class CppBundleGenerator(object):
  ...
  def __init__(self, root, model, api_defs, cpp_type_generator, cpp_namespace,
      extensions_include_path):
    self._root = root;
    self._model = model
    self._api_defs = api_defs
    self._cpp_type_generator = cpp_type_generator
    self._cpp_namespace = cpp_namespace
    self.api_cc_generator = _APICCGenerator(self, extensions_include_path)
...
class _APICCGenerator(object):
  ...
  def __init__(self, cpp_bundle, extensions_include_path):
    self._bundle = cpp_bundle
    self._extensions_include_path = (extensions_include_path
        if extensions_include_path else DEFAULT_EXTENSIONS_INCLUDE_PATH)
  ...
  def Generate(self, namespace):
    ...
    for namespace in self._bundle._model.namespaces.values():
      ...
      if (implementation_header):
        ...
        implementation_header = "%s/api/%s/%s_api.h" % (self._extensions_include_path, namespace_name, namespace_name)
    ...
    c.Append()
    c.Append('#include '
                 '"%s/extension_function_registry.h"' % self._extensions_include_path)
    c.Append()
    c.Concat(cpp_util.OpenNamespace(self._bundle._cpp_namespace))
    c.Append()
