...
void WriteValue(Message* m, const base::Value* value, int recursion) {
  ...
  switch (value->GetType()) {
    ...
    case base::Value::TYPE_INT64: {
      int64 val;
      result = value->GetAsInt64(&val);
      DCHECK(result);
      WriteParam(m, val);
      break;
    }
    ...
    case Value::TYPE_PICKLE: {
      base::PickleValue* pickle =
          const_cast<base::PickleValue*>(
              static_cast<const base::PickleValue*>(value));
      m->WriteData(static_cast<const char*>(pickle->GetPickle().data()),
                   pickle->GetPickle().size());
      break;
    }
  }
}
  ...
  switch (type) {
    ...
    *value = base::Value::CreateNullValue();
    ...
    case Value::TYPE_PICKLE: {
      ...
      *value = base::PickleValue::CreateWithCopiedBuffer(data, length);
      break;
    }
    default:
    return false;
