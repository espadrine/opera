...
// This structure is used to extend information about text element to proper
// handle it in Opera
struct CONTENT_EXPORT EditFormProperties {
  EditFormProperties();

  // True for text areas and text inputs.
  bool is_text_form;

  // True if element has read only attribute.
  bool is_read_only;

  // True if input type is password.
  bool is_password;

  // True if element has text inside.
  bool has_value;
};
...
struct CONTENT_EXPORT ContextMenuParams {
  ...
  // These structure tell more about text form elements such as text areas,
  // text inputs fields
  EditFormProperties edit_properties;
