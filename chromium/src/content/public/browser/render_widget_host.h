...
class CONTENT_EXPORT RenderWidgetHost : public IPC::Sender {
 public:
  ...
  virtual void SelectWord(const gfx::Point& position) = 0;
  ...
  // Request a callback to be invoked after an offscreen frame has been rendered.
  virtual void RequestOffscreenFrameCallback(
      const base::Callback<void()>& callback) = 0;

  // Request a callback to be invoked after a frame has been presented. If the
  // widget host has a widget host view, invoke the callback when its updated
  // view has been presented. If any previous frame callback request has not
  // yet been answered, it will be canceled.
  virtual void RequestFrameCallback(const base::Callback<void()>& callback) = 0;
