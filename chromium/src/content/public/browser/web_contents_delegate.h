...
class CONTENT_EXPORT WebContentsDelegate {
 public:
  ...
  // Media play state has changed.
  virtual void UpdatePlayState(WebContents* source,
...
virtual void OnSampledScrollSpeed(const gfx::Point& scroll_delta,
                                    const gfx::Point& start_position,
                                    const gfx::Point& view_size,
  ...
  // Called when a new query should be created and passed to the current search
  // engine. This is typically triggered by selecting a piece of text and
  // choosing to use it for search in the selection context menu.
  virtual void SearchUsingText(const string16& text) {}
  ...
  virtual void OverrideRendererPreferences(RendererPreferences* prefs) {}
  ...
  // A non-empty rendering tree has been activated by the render widget.
  virtual void NonEmptyTreeActivated() {}
