...
class NavigationController {
 public:
  ...
  struct CONTENT_EXPORT LoadURLParams {
    ...
    // Content state blob (generated by Blink) that the page will be restored
    // into. This blob is versioned, so a newer version of Blink should always
    // support content state blobs written by an older version. This blob keeps
    // information such as the scroll offset, form data, etc... It can be
    // acquired via the NavigationEntry::GetContentState() function.
    //
    // If left empty, content state restoration will not be done.
