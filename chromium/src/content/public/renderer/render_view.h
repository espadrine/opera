...
class CONTENT_EXPORT RenderView : public IPC::Sender {
 public:
  ...
  // TVSDK benchmarking: resets the frame counter.
  virtual void RestartFrameCount() {}

  // TVSDK benchmarking: gets the number of frame updates performed
  // since the frame count was last reset.
  ...
  virtual void SetSitePrefs(scoped_refptr<SitePrefs> site_prefs) = 0;
  ...
  // Halt memory consuming activities in the renderer ASAP
  // I.e. stop page loading, running scripts etc.
  // Used as a last resort when memory is running dangerously low.
  virtual void MemoryHalt() = 0;
