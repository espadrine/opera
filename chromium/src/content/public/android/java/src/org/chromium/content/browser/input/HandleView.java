...
public class HandleView extends View {
    ...
    // Due to the structure of the window layout, the window will be larger
    // than the content inside it. This value is used to position the window
    // without accounting for the empty space within it.
    private int mWindowPadding = 0;
    ...
    HandleView(CursorController controller, int pos, View parent) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            HoneycombCompatLayer.setSplitTouchEnabled(mContainer, true);
        }
    ...
    private void setIsDragging(boolean isDragging) {
        if (isDragging != mIsDragging) {
            ...
            if (mListener != null) {
                mListener.onDragStateChanged(mIsDragging);
            }
        }
    }
    ...
    void show() {
        ...
        mContainerPositionY = coords[1] + mPositionY - mWindowPadding;
    ...
    void moveTo(int x, int y) {
        ...
        if (isPositionVisible()) {
            ...
            if (mContainer.isShowing()) {
                ...
                final int containerPositionY = coords[1] + mPositionY - mWindowPadding;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mTouchToWindowOffsetY = mDownPositionY - mPositionY + mWindowPadding;
                final int[] coords = mTempCoords;
                mParent.getLocationInWindow(coords);
                mLastParentX = coords[0];
                mLastParentY = coords[1];
    ...
    /**
     * Specifies whether the handle is the anchor point or not.
     * @param isAnchor true if the handle is the anchor point, false if it's not.
     */
    ...
    /**
     * @return true if the handle is the anchor point, false if it's not.
     */
    ...
    private static class HoneycombCompatLayer {
        static void setSplitTouchEnabled(PopupWindow win, boolean enabled) {
            win.setSplitTouchEnabled(enabled);
        }
    }
}
