...
public class ChildProcessConnection implements ServiceConnection {
    ...
    private void doConnectionSetup() {
        ...
        if (onConnectionCallback == null) {
            ...
            FDParcelable[] parcelFiles = new FDParcelable[fileInfos.length];
            for (int i = 0; i < fileInfos.length; i++) {
                ...
                if (fileInfos[i].mAutoClose) {
                    // Adopt the FD, it will be closed when we close the file descriptor.
                    parcelFiles[i] = FDParcelable.adoptFd(fileInfos[i].mFd);
                } else {
                    try {
                        parcelFiles[i] = FDParcelable.fromFd(fileInfos[i].mFd);
                    } catch(IOException e) {
                        Log.e(TAG,
            ...
            try {
                for (FDParcelable parcelFile : parcelFiles) {
    ...
    private class HighPriorityConnection implements ServiceConnection {
        ...
        void bind() {
            ...
            mHBound = mContext.bindService(intent, this, flags);
        }
