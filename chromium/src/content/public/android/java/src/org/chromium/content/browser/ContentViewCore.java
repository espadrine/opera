...
public class ContentViewCore implements MotionEventDelegate, NavigationClient {
    ...
    private boolean mIsPendingSelection = false;

    private boolean mBrowserContextMenuShowing = false;
    ...
    // true if we're currently scrolling by dragging a selection or insertion
    // handle close to the view edges.
    private boolean mHandleScrolling = false;
    ...
    public ContentViewCore(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            mCompatLayer = new IceCreamSandwichMR1CompatLayer();
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mCompatLayer = new IceCreamSandwichCompatLayer();
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR1) {
            mCompatLayer = new HoneyCombMR1CompatLayer();
    ...
    public void initialize(ViewGroup containerView, InternalAccessDelegate internalDispatcher,
        ...
        mWebContentsObserver = new WebContentsObserverAndroid(this) {
            ...
            public void didStartLoading(String url) {
                hidePopupDialog();
                getContentViewClient().hidePopupDialog();
    ...
    /**
     * Initializes the selection popup menu.
     */
    private void initSelectMenu() {
        mSelectMenu = new SelectMenu(mContext, mContainerView) {
            @Override
            public void onCopy() {
                mImeAdapter.copy();
            ...
            public void onSearch() {
                nativeSearchUsingText(mNativeContentViewCore, getSelectedText());
                mImeAdapter.unselect();
            }
        };
    }
    ...
    /**
     * @return The SelectMenu for this ContentViewCore.
     */
    ...
    public void setBrowserContextMenuShowing(boolean showing) {
        if (mBrowserContextMenuShowing && !showing) {
            nativeNotifyContextMenuClosed(mNativeContentViewCore);
        }
        mBrowserContextMenuShowing = showing;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN && mNativeContentViewCore != 0) {
            nativeSetFocus(mNativeContentViewCore, hasFocus());
        }
        undoScrollFocusedEditableNodeIntoViewIfNeeded(false);
    ...
    public void find(int requestId, String searchString, boolean forward, boolean matchCase, boolean findNext) {
        if (mSelectionHandleController != null) {
            mSelectionHandleController.hideAndDisallowAutomaticShowing();
        }
        if (mNativeContentViewCore != 0) {
            nativeFind(mNativeContentViewCore, requestId, searchString, forward, matchCase, findNext);
        }
    }

    public void stopFinding(ContentView.StopFindAction findAction) {
        if (mNativeContentViewCore != 0) {
            nativeStopFinding(mNativeContentViewCore, findAction.ordinal());
        }
    }

    public void activateNearestFindResult(int requestId, float x, float y) {
        if (mNativeContentViewCore != 0) {
            nativeActivateNearestFindResult(mNativeContentViewCore, requestId, x, y);
        }
    }

    public void requestFindMatchRects(int currentVersion) {
        if (mNativeContentViewCore != 0) {
            nativeRequestFindMatchRects(mNativeContentViewCore, currentVersion);
        }
    }
    ...
    /**
     * Suspend all media playback.
     */
    public void suspendMedia() {
        if (mNativeContentViewCore != 0) {
            nativeSuspendMedia(mNativeContentViewCore);
        }
    }

    /**
     * Resume suspended media playback.
     */
    public void resumeSuspendedMedia() {
        if (mNativeContentViewCore != 0) {
            nativeResumeSuspendedMedia(mNativeContentViewCore);
        }
    }
    ...
    private void showSelectMenu() {
        getSelectMenu().enablePaste(mSelectionEditable && mCompatLayer.hasClipboardData());
        getSelectMenu().show();
    ...
    public void onSizeChanged(int wPix, int hPix, int owPix, int ohPix) {
        ...
        if (nativeUsingBrowserSideCompositor(mNativeContentViewCore))
    ...
    private void updateAfterSizeChanged() {
        ...
        if (!mFocusPreOSKViewportRect.isEmpty()) {
            ...
            nativeClearFocusedNode(mNativeContentViewCore);
    ...
    public void scrollToVerticalEdge(boolean scrollToTop) {
        nativeScrollToVerticalEdge(mNativeContentViewCore, scrollToTop);
    }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (mPopupZoomer.isShowing() && keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            return true;
        } else if (mHasSelection && keyCode == KeyEvent.KEYCODE_BACK) {
            mImeAdapter.unselect();
            hidePopupDialog();
            return true;
        }
        ...
        if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_SCROLL:
                    mCompatLayer.sendMouseWheelEvent(event);
    ...
    private void handleTapOrPress(
        ...
        if (isLongPressOrTap == IS_LONG_PRESS) {
            ...
            if (mHasSelection) mIsPendingSelection = true;
            if (mNativeContentViewCore != 0) {
    ...
    // Scrolls the view when dragging handle close to the edges.
    private void scrollEdge(int x, int y, int handleWidth, int handleHeight) {
        final int scrollAreaRelativeSize =
                mContext.getResources().getInteger(
                        R.integer.chromium_page_selection_scroll_area);
        final int scrollAreaWidth = mViewportWidthPix / scrollAreaRelativeSize;
        final int scrollAreaHeight = mViewportHeightPix / scrollAreaRelativeSize;

        // NOTE: Half the handle size is added to the bottom and horizontal
        // scroll areas to make it easier drag scrolling down/horizontally.
        // We could add the entire handle size but that makes it appaear as
        // if it's the "wrong" side of the handle that triggers scrolling.
        Rect inner = new Rect(
                scrollAreaWidth + handleWidth / 2, scrollAreaHeight,
                mViewportWidthPix - scrollAreaWidth - handleWidth / 2,
                mViewportHeightPix - scrollAreaHeight - handleHeight / 2);

        mHandleScrolling = !inner.contains(x, y);
        if (mHandleScrolling) {
            getContentViewClient().onHideSelectionPreview();
            ...
            final int scrollSpeedScaleFactor =
                    mContext.getResources().getInteger(
                            R.integer.chromium_page_selection_scroll_speed_factor);

            scrollTo(mRenderCoordinates.getScrollXPixInt() + dx / scrollSpeedScaleFactor,
                    mRenderCoordinates.getScrollYPixInt() + dy / scrollSpeedScaleFactor);
        }
    }
    ...
    private SelectionHandleController getSelectionHandleController() {
        if (mSelectionHandleController == null) {
            mSelectionHandleController = new SelectionHandleController(getContainerView()) {
                @Override
                public void swapSelectionAnchorAndFocus() {
                    if (mNativeContentViewCore != 0) {
                        nativeSwapSelectionAnchorAndFocus(mNativeContentViewCore);
                    }
                }

                @Override
                public void moveSelectionFocus(int x, int y) {
                    if (mNativeContentViewCore != 0) {
                        nativeMoveSelectionFocus(mNativeContentViewCore, x, y);
                        scrollEdge(x, y, getHandleWidth(), getHandleHeight());
                    }
                }
            ...
            mSelectionHandleController.setWindowPadding((int)mRenderCoordinates.getContentOffsetYPix());
            mSelectionHandleController.hideAndDisallowAutomaticShowing();
        }
        ...
        if (mInsertionHandleController == null) {
            mInsertionHandleController = new InsertionHandleController(getContainerView()) {
                ...
                public void showHandle() {
                    super.setWindowPadding((int)mRenderCoordinates.getContentOffsetYPix());
                    super.showHandle();
                }
            };
    ...
    private boolean isSelectMenuShowing() {
        return mSelectMenu != null && mSelectMenu.isShowing();
    }
    ...
    private void temporarilyHideTextHandles() {
        ...
        if (isSelectMenuShowing()) {
            getSelectMenu().setVisibility(View.INVISIBLE);
        }
            ...
            mDeferredHandleFadeInRunnable = new Runnable() {
                ...
                public void run() {
                    if (mContentViewGestureHandler.isNativeScrolling() ||
                            ...
                            nativeIsTopBarAnimating(mNativeContentViewCore)) {
                        // Delay fade in until no longer scrolling, pinching or
                        ...
                        if (isSelectMenuShowing()) {
                            getSelectMenu().setVisibility(View.VISIBLE);
                        }
                    }
                }
            };
    ...
    private void updateFrameInfo(
        ...
        final boolean topBarAnimating =
                nativeIsTopBarAnimating(mNativeContentViewCore);

        final boolean needHidePopupZoomer = contentSizeChanged || scrollChanged;
        final boolean needUpdateZoomControls = scaleLimitsChanged || scrollChanged;
        final boolean needTemporarilyHideHandles = scrollChanged || topBarAnimating;
        ...
        if (mSelectionHandleController != null)
            mSelectionHandleController.setWindowPadding((int)contentOffsetYPix);
        if (mInsertionHandleController != null)
            mInsertionHandleController.setWindowPadding((int)contentOffsetYPix);
    ...
    private void updateImeAdapter(int nativeImeAdapterAndroid, int textInputType,
        ...
        mCompatLayer.invalidateActionMode();
    ...
    private void showSelectionPreview(Bitmap previewBitmap, Rect focusRectDip) {
        if (mSelectionHandleController.isDragging()) {
    ...
    private void onSelectionBoundsChanged(Rect anchorRectDip, int anchorDir, Rect focusRectDip,
        ...
        if (x1 != x2 || y1 != y2 ||
            ...
            mHasSelection = true;
            if (mIsPendingSelection) {
                mSelectionHandleController.allowAutomaticShowing();
                mIsPendingSelection = false;
            }
            ...
            Rect boundingRect = new Rect(
                    Math.round(Math.min(mStartHandlePoint.getXPix(), mEndHandlePoint.getXPix())),
                    Math.round(Math.min(mStartHandlePoint.getYPix(), mEndHandlePoint.getYPix())),
                    Math.round(Math.max(mStartHandlePoint.getXPix(), mEndHandlePoint.getXPix())),
                    Math.round(Math.max(mStartHandlePoint.getYPix(), mEndHandlePoint.getYPix())));
            getSelectMenu().anchorAt(boundingRect);

            if (!mHandleScrolling) {
                if (mSelectionHandleController.isDragging()) {
                    Rect anchorRect = mRenderCoordinates.fromDipToPix(anchorRectDip);
                    Rect focusRect = mRenderCoordinates.fromDipToPix(focusRectDip);

                    getContentViewClient().onSelectionChanged(anchorRect, focusRect);
                } else if (mSelectionHandleController.isShowing()) {
                    showSelectMenu();
                }
            }
        } else {
     ...
     */
    public void onInitializeAccessibilityNodeInfo(Object info) {
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        mCompatLayer.initializeAccessibilityScrolling(event);
    }
     ...
     * @param onlyAtTop Whether showing the top controls should only be shown at the
                        top of the page.
     * @param animate Whether the transition should be animated or not.
     */
    public void updateTopControlsState(boolean enableHiding, boolean enableShowing,
            boolean onlyAtTop, boolean animate) {
        nativeUpdateTopControlsState(mNativeContentViewCore, enableHiding, enableShowing,
                onlyAtTop, animate);
    }
    ...
    public void forceRenderWidgetHostViewBackingSize(int width, int height) {
        if (mNativeContentViewCore != 0) {
            nativeForceRenderWidgetHostViewBackingSize(
                    mNativeContentViewCore, width, height);
        }
    }
    ...
    private interface CompatLayer {
        ...
        public void initializeAccessibilityScrolling(AccessibilityEvent event);
        public void sendMouseWheelEvent(MotionEvent event);
    ...
    private class GenericCompatLayer implements CompatLayer {
        ...
        @Override
        public boolean hasClipboardData() {
            android.text.ClipboardManager clipMgr = (android.text.ClipboardManager)
                    getContext().getSystemService(Context.CLIPBOARD_SERVICE);
            return clipMgr.hasText();
        }
    }

    private class HoneyCombCompatLayer extends GenericCompatLayer implements CompatLayer {
        ...
        public void showSelectActionBar() {
            ...
            if (mContainerView.getParent() != null) {
              ActionMode.Callback callback =
                (ActionMode.Callback)getContentViewClient().getSelectActionModeCallback(
                    getContext(), actionHandler, nativeIsIncognito(mNativeContentViewCore));
              mActionMode = mContainerView.startActionMode(callback);
            }
            mUnselectAllOnActionModeDismiss = true;
            if (mActionMode == null) {
        ...
        @Override
        public boolean hasClipboardData() {
            android.content.ClipboardManager clipMgr = (android.content.ClipboardManager)
                    getContext().getSystemService(Context.CLIPBOARD_SERVICE);
            return clipMgr.hasPrimaryClip();
        }
    }

    private class HoneyCombMR1CompatLayer extends HoneyCombCompatLayer implements CompatLayer {
        public void sendMouseWheelEvent(MotionEvent event) {
    ...
    private int calcMaxScrollX() {
        return Math.max(0, mRenderCoordinates.getMaxHorizontalScrollPixInt());
    }

    private int calcMaxScrollY() {
        return Math.max(0, mRenderCoordinates.getMaxVerticalScrollPixInt());
    }

    private class IceCreamSandwichCompatLayer extends HoneyCombMR1CompatLayer implements CompatLayer {
        @Override
        public void initializeAccessibilityScrolling(AccessibilityEvent event) {
            ...
            event.setScrollable(calcMaxScrollX() > 0 || calcMaxScrollY() > 0);
        }
    }

    private class IceCreamSandwichMR1CompatLayer extends IceCreamSandwichCompatLayer implements CompatLayer {
        @Override
        public void initializeAccessibilityScrolling(AccessibilityEvent event) {
            super.initializeAccessibilityScrolling(event);

            event.setMaxScrollX(calcMaxScrollX());
            event.setMaxScrollY(calcMaxScrollY());
        }
    }
    ...
    private native void nativeSwapSelectionAnchorAndFocus(int nativeContentViewCoreImpl);

    private native void nativeMoveSelectionFocus(int nativeContentViewCoreImpl, int x, int y);

    private native void nativeMoveCaret(int nativeContentViewCoreImpl, float x, float y);

    private native void nativeRequestSelectionPreview(int nativeContentViewCoreImpl,
    ...
    private native void nativeNotifyContextMenuClosed(int nativeContentViewCoreImpl);
    ...
    private native void nativeFind(int nativeContentViewCoreImpl,
            int requestId, String searchString, boolean forward, boolean matchCase, boolean findNext);

    private native void nativeStopFinding(int nativeContentViewCoreImpl, int findAction);

    private native void nativeActivateNearestFindResult(int nativeContentViewCoreImpl, int requestId, float x, float y);

    private native void nativeRequestFindMatchRects(int nativeContentViewCoreImpl, int currentVersion);
    ...
    private native void nativeSuspendMedia(int nativeContentViewCoreImpl);
    private native void nativeResumeSuspendedMedia(int nativeContentViewCoreImpl);
    ...
    private native void nativeSearchUsingText(int nativeContentViewCoreImpl, String text);
    ...
    private native void nativeScrollToVerticalEdge(int nativeContentViewCoreImpl, boolean scrollToTop);

    private native void nativeWasResized(int nativeContentViewCoreImpl);

    private native boolean nativeUsingBrowserSideCompositor(int nativeContentViewCoreImpl);
    ...
    private native void nativeUpdateTopControlsState(int nativeContentViewCoreImpl,
            boolean enableHiding, boolean enableShowing, boolean onlyAtTop, boolean animate);
    private native boolean nativeIsTopBarAnimating(int nativeContentViewCoreImpl);

    private native void nativeClearFocusedNode(int nativeContentViewCoreImpl);
    ...
    private native void nativeForceRenderWidgetHostViewBackingSize(
            int nativeContentViewCoreImpl, int width, int height);
}
