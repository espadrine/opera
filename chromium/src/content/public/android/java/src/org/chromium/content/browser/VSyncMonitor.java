...
public class VSyncMonitor {
    ...
    // Choreographer is used to detect vsync on >= JB (in compatibility layer).
    ...
    VSyncMonitor(Context context, VSyncMonitor.Listener listener, boolean enableJBVSync) {
        ...
        if (enableJBVSync && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            mCompatLayer = new JellyBeanCompatLayer();
        } else {
            mCompatLayer = new CompatLayer();
        }

        mCompatLayer.initVSync();
    ...
    public boolean isVSyncSignalAvailable() {
        return mCompatLayer.isVSyncSignalAvailable();
    }
    ...
    private void postCallback() {
        ...
        if (isVSyncSignalAvailable()) {
            mCompatLayer.postFrameCallback();
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
    ...
    private class CompatLayer {
        void initVSync() {}
        boolean isVSyncSignalAvailable() {
            return false;
        }
        void postFrameCallback() {}
    }

    private class JellyBeanCompatLayer extends CompatLayer {
