...
public class LibraryLoader {
    ...
    // Additional libraries to load which are not known at compile time
    private static List<String> additionalLibraries = new ArrayList<String>();
    ...
    /**
     * Add additional library to load.
     * Usable for libraries which we only know at runtime
     *
     * @param libraryPath full path to so file to load
     */
    public static void addAdditionalLibrary(String libraryPath) {
        additionalLibraries.add(libraryPath);
    }
    ...
    /**
     *  This method blocks until the library is fully loaded and initialized;
     *  however, it won't load it itself.
     */
    ...
    private static void loadAlreadyLocked() throws ProcessInitException {
        try {
            if (!sLoaded) {
                ...
                for (String sLibrary : additionalLibraries) {
                    Log.i(TAG, "loading: " + sLibrary);
                    if (sLibrary.endsWith(".so")) {
                        System.load(sLibrary);
                    } else {
                        System.loadLibrary(sLibrary);
                    }
                    Log.i(TAG, "loaded: " + sLibrary);
