...
scoped_ptr<RenderWidgetCompositor> RenderWidgetCompositor::Create(
  ...
  settings.frame_driven_animation_enabled =
      cmd->HasSwitch(switches::kDisableBrowserSideCompositor);
  settings.dont_draw_when_resizing =
      cmd->HasSwitch(switches::kDisableBrowserSideCompositor);

  settings.max_untiled_layer_size = gfx::Size(max_untiled_layer_width,
                                           max_untiled_layer_height);

  settings.top_controls_adjust_viewport =
      cmd->HasSwitch(switches::kDisableBrowserSideCompositor);
...
RenderWidgetCompositor::RenderWidgetCompositor(RenderWidget* widget)
  : suppress_schedule_composite_(false),
    widget_(widget),
    compositor_filter_(
        RenderThreadImpl::current()->compositor_filter()) {
  if (compositor_filter_.get()) {
    compositor_proxy_ = new CompositorProxy(this);
    compositor_filter_->AddRoute(
        widget->routing_id(),
        base::Bind(&CompositorProxy::OnMessageReceived, compositor_proxy_));
  }
}

RenderWidgetCompositor::~RenderWidgetCompositor() {
  if (compositor_proxy_)
    compositor_proxy_->Reset();
  if (compositor_filter_.get())
    compositor_filter_->RemoveRoute(widget_->routing_id());
}
...
IPC::ForwardingMessageFilter* RenderWidgetCompositor::CreateFilter(
...
void RenderWidgetCompositor::OnMessageReceived(const IPC::Message& message) {
  IPC_BEGIN_MESSAGE_MAP(RenderWidgetCompositor, message)
    IPC_MESSAGE_HANDLER(ViewMsg_Frame, OnFrameOnImplThread);
  IPC_END_MESSAGE_MAP()
}

void RenderWidgetCompositor::OnFrameOnImplThread(
    const ViewMsg_Frame_Params& params) {
  if (params.force)
    layer_tree_host_->ForceFrameOnImplThread();
  if (params.schedule)
    layer_tree_host_->ScheduleFrameOnImplThread();
}
...
void RenderWidgetCompositor::NonEmptyTreeActivated() {
  ChildThread::current()->sync_message_filter()->Send(
      new ViewHostMsg_NonEmptyTreeActivated(widget_->routing_id()));
}
...
void RenderWidgetCompositor::DidCompleteScale() {
  widget_->webwidget()->didCompleteScale();
}

void RenderWidgetCompositor::DidPinchGesture(float scale, const gfx::Point& p) {
  widget_->webwidget()->didPinchGesture(scale, p);
}

void RenderWidgetCompositor::MaxScrollUpdated(const gfx::Vector2d& maxScroll) {
  widget_->webwidget()->maxScrollUpdated(maxScroll);
}

void RenderWidgetCompositor::PendingTreeActivated(
    const gfx::Vector2d& scrollTotal) {
  widget_->webwidget()->pendingTreeActivated(scrollTotal);
}
...
void RenderWidgetCompositor::StartFrameLoopOnImplThread() {
  ChildThread::current()->sync_message_filter()->Send(
      new ViewHostMsg_StartFrameLoop(widget_->routing_id()));
}

void RenderWidgetCompositor::StopFrameLoopOnImplThread() {
  ChildThread::current()->sync_message_filter()->Send(
      new ViewHostMsg_StopFrameLoop(widget_->routing_id()));
}

void RenderWidgetCompositor::InvalidateOnImplThread() {
  ChildThread::current()->sync_message_filter()->Send(
      new ViewHostMsg_Invalidate(widget_->routing_id()));
}

void RenderWidgetCompositor::Invalidate() {
  widget_->Invalidate();
}
...
void RenderWidgetCompositor::DidChangeTopControlsAnimationState(bool active) {
  widget_->DidChangeTopControlsAnimationState(active);
}
...
void RenderWidgetCompositor::ScheduleFrame() {
  layer_tree_host_->ScheduleFrame();
}
