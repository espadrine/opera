...
RenderWidget::RenderWidget(WebKit::WebPopupType popup_type,
    ...
    : routing_id_(MSG_ROUTING_NONE),
      ...
      pending_update_selection_bounds_(false),
      is_animating_top_bar_(false),
#if defined(OS_ANDROID)
      current_selection_preview_dib_(NULL),
#endif
...
void RenderWidget::Resize(const gfx::Size& new_size,
  ...
  // The implicit post-resize frame.
  if (compositor_)
    compositor_->ScheduleFrame();
}
...
double RenderWidget::GetSecondsPerFrameCap() const {
  NOTIMPLEMENTED();
  return 0.0;
}

bool RenderWidget::HasSecondsPerFrameCap() const {
  return false;
}
...
void RenderWidget::AnimationCallback() {
  ...
  if (!animation_floor_time_.is_null() && IsRenderingVSynced()) {
    ...
    base::TimeDelta frame_interval = HasSecondsPerFrameCap() ?
        base::TimeDelta::FromMilliseconds(
            static_cast<int>(GetSecondsPerFrameCap() * 1000)) :
        base::TimeDelta::FromMilliseconds(16);
    base::TimeDelta animation_callback_delay = base::Time::Now() -
        (animation_floor_time_ - frame_interval);
...
void RenderWidget::AnimateIfNeeded() {
  ...
  base::TimeDelta animationInterval = HasSecondsPerFrameCap() ?
      base::TimeDelta::FromMilliseconds(
          static_cast<int>(GetSecondsPerFrameCap() * 1000)) :
      (IsRenderingVSynced() ?
...
void RenderWidget::DoDeferredUpdate() {
  ...
  if (!is_accelerated_compositing_active_ &&
      HasSecondsPerFrameCap() &&
      animation_timer_.IsRunning()) {
    TRACE_EVENT0("renderer", "EarlyOut_AnimationPending");
    return;
  }
...
void RenderWidget::Invalidate() {
  Send(new ViewHostMsg_Invalidate(routing_id_));
}
...
void RenderWidget::OnRequestSelectionPreview(const gfx::Rect& focus_rect,
  ...
  if (current_selection_preview_dib_ == NULL) {
...
void RenderWidget::OnShowSelectionPreviewComplete() {
  if (current_selection_preview_dib_) {
    RenderProcess::current()->ReleaseTransportDIB(
        current_selection_preview_dib_);
    current_selection_preview_dib_ = NULL;
  }
  if (!pending_selection_preview_.focus_rect.IsEmpty()) {
...
void RenderWidget::UpdateTextInputState(ShowIme show_ime) {
  ...
  bool likely_username = false;
  if (webwidget_)
    new_info = webwidget_->textInputInfo(&likely_username);
  ...
  if (show_ime_if_needed || (text_input_type_ != new_type
    ...
    p.type =
#if defined(OS_ANDROID)
        likely_username ? ui::TEXT_INPUT_TYPE_EMAIL :
...
void RenderWidget::UpdateSelectionBounds() {
  ...
  bool selection_changed = selection_anchor_rect_ != params.anchor_rect ||
                           selection_focus_rect_ != params.focus_rect;

  if (is_animating_top_bar_) {
    if (selection_changed) {
      pending_update_selection_bounds_ = true;
    }
  } else {
    if (selection_changed || pending_update_selection_bounds_) {
      pending_update_selection_bounds_ = false;
...
#if defined(OS_ANDROID)
gfx::Size RenderWidget::CreateZoomedBitmap(
    const gfx::Rect& bounds_rect,
    float scale,
    TransportDIB** transport_dib) {
  gfx::Size canvas_size = gfx::ToCeiledSize(
      gfx::ScaleSize(bounds_rect.size(),
  ...
  canvas->drawColor(webwidget_->backgroundColor());

  webwidget_->paint(webkit_glue::ToWebCanvas(canvas.get()), bounds_rect,
      WebWidget::ForceSoftwareRenderingAndIgnoreGPUResidentContent);

  return canvas_size;
}

void RenderWidget::ShowSelectionPreview(
    const gfx::Rect& focus_rect,
    const gfx::Size& preview_size,
    float scale_treshold) {
  WebKit::WebView* webview = static_cast<WebKit::WebView*>(webwidget_);

  gfx::SizeF dip_preview_size =
      gfx::ScaleSize(preview_size, 1.0 / device_scale_factor_);
  gfx::RectF client_focus_rect =
      gfx::ScaleRect(focus_rect, 1.0 / webview->pageScaleFactor());
  ...
  gfx::Size bitmap_size = CreateZoomedBitmap(
      client_rect, scale, &current_selection_preview_dib_);
  if (current_selection_preview_dib_) {
    Send(new ViewHostMsg_ShowSelectionPreview(
        routing_id_, bitmap_size, current_selection_preview_dib_->id(),
        focus_rect));
  }
}
#endif

void RenderWidget::DidChangeTopControlsAnimationState(bool active) {
  is_animating_top_bar_ = active;
}
...
void RenderWidget::setInputMethodState(bool enabled) {
  UpdateTextInputType();
#if defined(OS_ANDROID)
  UpdateTextInputState(enabled ? SHOW_IME_IF_NEEDED : DO_NOT_SHOW_IME);
#endif
}
