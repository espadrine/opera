...
class CONTENT_EXPORT RenderViewImpl
 ...
 public:
...
#endif
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  webkit_media::WebMediaPlayerManagerExternal* media_player_manager() {
    return media_player_manager_.get();
  }
#endif
  ...
  virtual void playStateChanged(WebKit::WebMediaPlayState play_state);
  ...
  virtual void mediaBlocked(bool isVideo) OVERRIDE;
  virtual void mediaUnblocked(bool isVideo) OVERRIDE;
  virtual WebKit::WebView* createMediaBlockedWebView(
      WebKit::WebView* page_web_view,
      WebKit::WebViewClient* web_view_client,
      WebKit::WebFrameClient* web_frame_client) OVERRIDE;
  ...
  virtual void RestartFrameCount() OVERRIDE;
  ...
  virtual void TurboContentBlocked(
      TurboContentSettingsType settings_type,
      const std::string& resource_identifier) OVERRIDE;
  virtual void BlockedTurboContentRemoved(
      TurboContentSettingsType settings_type,
      const std::string& resource_identifier) OVERRIDE;
  ...
  virtual void SetSitePrefs(scoped_refptr<SitePrefs> site_prefs) OVERRIDE;
  ...
  virtual void MemoryHalt() OVERRIDE;
 ...
 protected:
  ...
  virtual double GetSecondsPerFrameCap() const OVERRIDE;
  virtual bool HasSecondsPerFrameCap() const OVERRIDE;
 ...
 private:
  ...
  void OnLoadBlockedByTurboContent();
  ...
  void OnScriptEvalRequest(const string16& frame_xpath,
                           ...
                           int id,
                           bool notify_result);
  void OnSelectWord(const gfx::Point& position);
  ...
  void OnSuspendMedia();
  void OnResumeSuspendedMedia();
  void OnSwapOut();
  void OnSwapSelectionAnchorAndFocus();
  void OnMoveSelectionFocus(const gfx::Point& point);
  void OnThemeChanged();
  ...
  // Start a scroll animation to the top or bottom of the view
  void OnScrollToVerticalEdgeOfView(bool scrollToTop);
  ...
  // TVSDK benchmarking: This stores the number of times the view has
  // been updated. See RenderView::GetFrameCount() and
  // RenderView::RestartFrameCount()
  int frame_update_count_;
...
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  // The media player manager for managing all the media players on this view.
  scoped_ptr<webkit_media::WebMediaPlayerManagerExternal> media_player_manager_;
#endif // OPERA_TVSDK
  ...
  // Minimum seconds to wait between frames.
  double seconds_per_frame_cap_;
  ...
  scoped_refptr<SitePrefs> site_prefs_;
  ...
  ScrollOffsetSampler* scroll_offset_sampler_;
