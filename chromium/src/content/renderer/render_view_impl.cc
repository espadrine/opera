...
#include "content/public/renderer/site_prefs.h"
...
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
#include "webkit/media/externalplayer/webmediaplayer_manager_external.h"
#endif // OPERA_TVSDK
...
RenderViewImpl::RenderViewImpl(RenderViewImplParams* params)
    : RenderWidget(WebKit::WebPopupTypeNone,
      ...
      did_tap_multiple_targets_strategy_(MULTITARGET_ZOOM),
      is_loading_(false),
      ...
      seconds_per_frame_cap_(0.0),
      next_snapshot_id_(0),
      allow_partial_swap_(params->allow_partial_swap),
      scroll_offset_sampler_(NULL),
...
void RenderViewImpl::Initialize(RenderViewImplParams* params) {
...
#elif defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  media_player_manager_.reset(
      webkit_media::WebMediaPlayerManagerExternal::Create(this));
#endif
  ...
  scroll_offset_sampler_ = new ScrollOffsetSampler(this);
}
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderViewImpl, message, msg_is_ok)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SelectWord, OnSelectWord)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SwapSelectionAnchorAndFocus, OnSwapSelectionAnchorAndFocus)
    IPC_MESSAGE_HANDLER(ViewMsg_MoveSelectionFocus, OnMoveSelectionFocus)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SuspendMedia, OnSuspendMedia)
    IPC_MESSAGE_HANDLER(ViewMsg_ResumeSuspendedMedia, OnResumeSuspendedMedia)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_LoadBlockedByTurboContent,
                        OnLoadBlockedByTurboContent)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_ScrollToVerticalEdgeOfView,
                        OnScrollToVerticalEdgeOfView)
...
void RenderViewImpl::OnNavigate(const ViewMsg_Navigate_Params& params) {
  ...
  // Force dropping "Turbo allow load" state at reload or navigate
  webview()->settings()->setMediaBlockingEnabled(
      webkit_preferences_.media_blocking_enabled);
...
void RenderViewImpl::OnSelectWord(const gfx::Point& position) {
  if (!webview())
    return;

  webview()->focusedFrame()->selectWordAroundPosition(position);
...
void RenderViewImpl::OnSwapSelectionAnchorAndFocus() {
  if (!webview())
    return;

  webview()->focusedFrame()->swapSelectionBaseAndExtent();
}

void RenderViewImpl::OnMoveSelectionFocus(const gfx::Point& point) {
  if (!webview())
    return;

  Send(new ViewHostMsg_MoveSelectionFocus_ACK(routing_id_));

  webview()->focusedFrame()->moveSelectionExtent(point);
...
void RenderViewImpl::showContextMenu(
  ...
  params.x *= webview()->pageScaleFactor();
  params.y *= webview()->pageScaleFactor();
#endif
  ...
  if (webview())
    webview()->onContextMenuShown(data);
}
...
WebMediaPlayer* RenderViewImpl::createMediaPlayer(
...
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  return media_player_manager_->CreateMediaPlayer(frame, client);
#endif // OPERA_TVSDK
#if defined(OS_ANDROID)
  ...
  content::StreamTextureFactory* stream_texture_factory = NULL;
  if (media::IsSurfaceTextureSupported())
    stream_texture_factory = new StreamTextureFactory(
...
void RenderViewImpl::SetSitePrefs(scoped_refptr<SitePrefs> site_prefs) {
  site_prefs_ = site_prefs;
}
...
void RenderViewImpl::MemoryHalt() {
  VLOG(2) << "Allocated before MemoryHalt: "
          << content::MemoryAllowanceAgent::GetInstance()->GetAllocated();
  OnStop();
  VLOG(2) << "Allocated after MemoryHalt: "
          << content::MemoryAllowanceAgent::GetInstance()->GetAllocated();
}
...
WebKit::WebString RenderViewImpl::userAgentOverride(
  ...
  // TODO(mathiash): this is essentially an override of an override, we should
  // somehow work it into the existing override using renderer_preferences_ and
  // document state / navigation entries instead.
  if (site_prefs_) {
    std::string site_pref_user_agent = site_prefs_->GetUserAgent(GURL(url));
    if (!site_pref_user_agent.empty())
      return WebString::fromUTF8(site_pref_user_agent);
  }
...
void RenderViewImpl::RestartFrameCount() {
  frame_update_count_ = 0;
}

int RenderViewImpl::GetFrameCount() const {
  return frame_update_count_;
}

int RenderView::GetFrameCount() const {
  return 0;
}
...
void RenderViewImpl::TurboContentBlocked(
    TurboContentSettingsType settings_type,
    const std::string& resource_identifier) {
  Send(new ViewHostMsg_TurboContentBlocked(routing_id_, settings_type,
                                           resource_identifier));
}

void RenderViewImpl::BlockedTurboContentRemoved(
    TurboContentSettingsType settings_type,
    const std::string& resource_identifier) {
  Send(new ViewHostMsg_BlockedTurboContentRemoved(routing_id_,
                                                  settings_type,
                                                  resource_identifier));
}
...
void RenderViewImpl::OnSuspendMedia() {
  webview()->suspendMedia();
}

void RenderViewImpl::OnResumeSuspendedMedia() {
  webview()->resumeSuspendedMedia();
}
...
void RenderViewImpl::OnLoadBlockedByTurboContent() {
  FOR_EACH_OBSERVER(RenderViewObserver, observers_,
                    LoadBlockedByTurboContent());
  webview()->loadBlockedMediaContent();
  // Don't block anything after user allowed load.
  webview()->settings()->setMediaBlockingEnabled(false);
}
...
void RenderViewImpl::OnSetRendererPrefs(
  ...
  seconds_per_frame_cap_ = renderer_prefs.seconds_per_frame_cap;
}
...
#if defined(OS_ANDROID) || (defined(OPERA_TVSDK) && defined(ENABLE_UVA))
...
double RenderViewImpl::GetSecondsPerFrameCap() const {
  return seconds_per_frame_cap_;
}

bool RenderViewImpl::HasSecondsPerFrameCap() const {
  return seconds_per_frame_cap_ > 0.0;
}
...
void RenderViewImpl::playStateChanged(WebKit::WebMediaPlayState play_state) {
...
void RenderViewImpl::mediaBlocked(bool isVideo) {
  TurboContentSettingsType settings_type = isVideo ?
        TURBO_CONTENT_SETTINGS_TYPE_HTML5_VIDEO :
        TURBO_CONTENT_SETTINGS_TYPE_HTML5_AUDIO;
  TurboContentBlocked(settings_type, std::string());
}

void RenderViewImpl::mediaUnblocked(bool isVideo) {
  TurboContentSettingsType settings_type = isVideo ?
        TURBO_CONTENT_SETTINGS_TYPE_HTML5_VIDEO :
        TURBO_CONTENT_SETTINGS_TYPE_HTML5_AUDIO;
  BlockedTurboContentRemoved(settings_type, std::string());
}

WebKit::WebView* RenderViewImpl::createMediaBlockedWebView(
      WebKit::WebView* page_web_view,
      WebKit::WebViewClient* web_view_client,
      WebKit::WebFrameClient* web_frame_client) {
    const char* const kPluginPlaceholderDataURL =
        "chrome://pluginplaceholderdata/";
    WebKit::WebView* result = WebKit::WebView::create(web_view_client);
    result->initializeMainFrame(web_frame_client);
    std::string html_data =
        GetContentClient()->renderer()->GetTurboBlockedHtml();
    result->mainFrame()->loadHTMLString(html_data,
        GURL(kPluginPlaceholderDataURL));
    webkit_glue::ApplyWebPreferences(webkit_preferences_, result);
    return result;
}
...
void RenderViewImpl::OnScrollToVerticalEdgeOfView(bool scrollToTop) {
  if (webview())
    webview()->scrollToEdgeOfView(scrollToTop);
}
