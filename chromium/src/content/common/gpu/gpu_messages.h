...
IPC_STRUCT_BEGIN(GpuHostMsg_AcceleratedWidgetBuffersSwapped_Params)
  IPC_STRUCT_MEMBER(int32, surface_id)
  IPC_STRUCT_MEMBER(gfx::Size, next_buffer_size)
  IPC_STRUCT_MEMBER(bool, inhibit_frame)
  IPC_STRUCT_MEMBER(bool, was_offscreen)
IPC_STRUCT_END()
...
IPC_STRUCT_BEGIN(GpuHostMsg_RequestBitmapResult_Params)
  IPC_STRUCT_MEMBER(int32, surface_id)
  IPC_STRUCT_MEMBER(int32, route_id)
  IPC_STRUCT_MEMBER(int32, request_id)
  IPC_STRUCT_MEMBER(int32, bitmap_id)
IPC_STRUCT_END()
...
// Tells the entity who sent GpuHostMsg_AcceleratedWidgetAcquire that a surface
// with the given id has an associated native widget available for use. If the
// surface_id is 0 (zero) it means no widget was acquired.
IPC_MESSAGE_ROUTED1(AcceleratedWidgetMsg_Acquired,
                    int32 /* surface_id */)

// Tells the entity who previously received a AcceleratedWidgetMsg_Acquired
// that it now is required to release the widget and then send a
// GpuHostMsg_AcceleratedWidgetRelease message.
IPC_MESSAGE_ROUTED0(AcceleratedWidgetMsg_Release)

// This is a response to GpuHostMsg_RequestBitmapResult. It must be sent when
// the shared memory (the bitmap data) is no longer used so that it can be
// free:d (otherwise we will leak memory).
IPC_MESSAGE_ROUTED1(AcceleratedWidgetMsg_RequestBitmapResultACK,
                    int32 /* request_id */)
...
// Request access to a shared widget. If a widget is available, or made
// available before any other clients tries to acquire, a
// AcceleratedWidgetMsg_Acquired message will be sent to the given route_id.
IPC_MESSAGE_CONTROL2(GpuHostMsg_AcceleratedWidgetAcquire,
                     int32 /* route_id */,
                     int32 /* surface_id */)

// Tell the browser that a widget previously acquired has now been released
// and can be used by some other entity.
IPC_MESSAGE_CONTROL1(GpuHostMsg_AcceleratedWidgetRelease,
                     int32 /* surface_id */)

// Tell the browser that a widget has had its buffers swapped.
IPC_MESSAGE_CONTROL1(GpuHostMsg_AcceleratedWidgetBuffersSwapped,
                     GpuHostMsg_AcceleratedWidgetBuffersSwapped_Params)
...
// Tell the browser that a bitmap snapshot that was requested is ready.
IPC_MESSAGE_CONTROL1(GpuHostMsg_RequestBitmapResult,
                     GpuHostMsg_RequestBitmapResult_Params)

// Tell the browser that it needs to recomposite the given surface.
IPC_MESSAGE_CONTROL1(GpuHostMsg_ScheduleComposite,
                     int32 /* surface_id */)

// Tell the browser to force schedule a compositor frame.
IPC_MESSAGE_CONTROL1(GpuHostMsg_ForceCompositorFrame,
                     int32 /* surface_id */)
...
// Send to stub when output surface is to be activated.
IPC_MESSAGE_ROUTED0(GpuCommandBufferMsg_ActivateOutputSurface)
...
// Makes the command buffer request a bitmap from its surface.
IPC_MESSAGE_ROUTED3(GpuCommandBufferMsg_RequestBitmap,
                    int32 /* request_id */,
                    gfx::Size /* size */,
                    bool /* wait_for_frame */)
