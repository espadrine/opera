...
bool CommandBufferProxyImpl::ActivateOutputSurface() {
  if (last_state_.error != gpu::error::kNoError)
    return false;

  return Send(new GpuCommandBufferMsg_ActivateOutputSurface(route_id_));
}
...
bool CommandBufferProxyImpl::RequestBitmap(int32 request_id,
                                           const gfx::Size& size,
                                           bool wait_for_frame) {
  if (last_state_.error != gpu::error::kNoError)
    return false;

  return Send(new GpuCommandBufferMsg_RequestBitmap(
      route_id_, request_id, size, wait_for_frame));
}
