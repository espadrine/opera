...
#include "content/public/common/turbo_content_settings_types.h"
...
#include "third_party/WebKit/public/web/WebMediaPlayState.h"
...
IPC_ENUM_TRAITS(WebKit::WebMediaPlayState)
...
IPC_ENUM_TRAITS(TurboContentSettingsType)
...
IPC_STRUCT_TRAITS_BEGIN(content::ContextMenuParams)
  ...
  IPC_STRUCT_TRAITS_MEMBER(edit_properties)
...
IPC_STRUCT_TRAITS_BEGIN(content::EditFormProperties)
  IPC_STRUCT_TRAITS_MEMBER(is_password)
  IPC_STRUCT_TRAITS_MEMBER(is_read_only)
  IPC_STRUCT_TRAITS_MEMBER(is_text_form)
  IPC_STRUCT_TRAITS_MEMBER(has_value)
IPC_STRUCT_TRAITS_END()
...
IPC_STRUCT_TRAITS_BEGIN(content::RendererPreferences)
  ...
  IPC_STRUCT_TRAITS_MEMBER(seconds_per_frame_cap)
IPC_STRUCT_TRAITS_END()
...
#ifdef OPERA_TVSTORE
IPC_STRUCT_BEGIN(ViewHostMsg_LoadTVApp_Params)
  // Name of the application.
  IPC_STRUCT_MEMBER(std::string, name)

  // The (decrypted) URL of the application.
  IPC_STRUCT_MEMBER(GURL, application)

  // The URL of the icon corresponding to this application.
  IPC_STRUCT_MEMBER(GURL, icon_url)

  // List of (decrypted) URLs that this application might load.
  IPC_STRUCT_MEMBER(std::vector<GURL>, whitelist)

  // List of TV Store permission strings granted to this application.
  IPC_STRUCT_MEMBER(std::vector<std::string>, permissions)

  // Mode that the application should be started in.
  // TODO(mostynb): switch to an enum
  IPC_STRUCT_MEMBER(unsigned, mode)

  // True if this application supports side-by-side mode.
  IPC_STRUCT_MEMBER(bool, sbs_support)

  // The URL used to show broadcast video in side-by-side mode.
  // Note: not a GURL, in case we need to use a non-standard scheme.
  IPC_STRUCT_MEMBER(std::string, broadcast_url)
IPC_STRUCT_END()
#endif
...
IPC_STRUCT_BEGIN(ViewMsg_Frame_Params)
  IPC_STRUCT_MEMBER(bool, schedule)
  IPC_STRUCT_MEMBER(bool, force)
IPC_STRUCT_END()
...
// Tell the render widget that the associated output surface should be
// activated, if applicable. This message may not be sent before
// ViewHostMsg_OutputSurfaceBound has been received.
IPC_MESSAGE_ROUTED0(ViewMsg_ActivateOutputSurface)

// Notifies the widget host that the output surface has been bound.
IPC_MESSAGE_ROUTED0(ViewHostMsg_OutputSurfaceBound)

// Notify the renderer that a frame has been scheduled. Only used when
// the browser side compositor is disabled.
IPC_MESSAGE_ROUTED1(ViewMsg_Frame,
                    ViewMsg_Frame_Params /* params */)

// Notify the browser that the view has been invalidated and needs to be
// redrawn. The browser should reply with a ViewMsg_Frame message when
// appropriate, taking in back buffer resizing into account. Only used when
// the browser side compositor is disabled.
IPC_MESSAGE_ROUTED0(ViewHostMsg_Invalidate)

// Notify the browser that the first non-empty rendering tree has been
// activated by the compositor.
IPC_MESSAGE_ROUTED0(ViewHostMsg_NonEmptyTreeActivated)

// Notify the browser that the renderer wants a constant flow, vsync:ed if
// possible, of ViewMsg_Frame messages. The browser needs to take back buffer
// resizing into account. Only used when the browser side compositor is
// disabled.
IPC_MESSAGE_ROUTED0(ViewHostMsg_StartFrameLoop)
IPC_MESSAGE_ROUTED0(ViewHostMsg_StopFrameLoop)
...
// Requests the renderer to select word at given position.
IPC_MESSAGE_ROUTED1(ViewMsg_SelectWord,
                    gfx::Point /* position */)
...
// Requests the renderer to swap the selection anchor and focus points.
IPC_MESSAGE_ROUTED0(ViewMsg_SwapSelectionAnchorAndFocus)

// Requests the renderer to move the selection focus point to a new position.
IPC_MESSAGE_ROUTED1(ViewMsg_MoveSelectionFocus,
                    gfx::Point /* point */)
...
// Suspend and resume media playback.
IPC_MESSAGE_ROUTED0(ViewMsg_SuspendMedia)
IPC_MESSAGE_ROUTED0(ViewMsg_ResumeSuspendedMedia)
...
IPC_MESSAGE_ROUTED4(ViewMsg_UpdateTopControlsState,
                    bool /* enable_hiding */,
                    bool /* enable_showing */,
                    bool /* only_at_top */,
                    bool /* animate */)
...
// Requests a bitmap of the current view
IPC_MESSAGE_ROUTED3(ViewMsg_RequestBitmap,
                    int /* request_id */,
                    gfx::Size /* size */,
                    bool /* wait_for_frame */)
...
#ifdef OPERA_TVSTORE
IPC_MESSAGE_CONTROL1(ViewHostMsg_LoadTVApp, ViewHostMsg_LoadTVApp_Params)
#endif
...
// Notifies the browser about media play state changes.
// This is an aggregate state per view.
IPC_MESSAGE_ROUTED1(ViewHostMsg_UpdatePlayState,
                    WebKit::WebMediaPlayState /* play_state */)
...
IPC_MESSAGE_ROUTED0(ViewHostMsg_MoveSelectionFocus_ACK)
...
// Sent from browser process to renderer to unblock all content in it,
// which was blocked by turbo.
IPC_MESSAGE_ROUTED0(ViewMsg_LoadBlockedByTurboContent)

// Sent by the renderer process to tell browser that it blocked some object
// because of turbo is turned on.
IPC_MESSAGE_ROUTED2(ViewHostMsg_TurboContentBlocked,
                    TurboContentSettingsType, /* type of blocked content */
                    std::string /* resource identifier */)

// Sent by the renderer process to tell browser that some content, which was
// blocked by turbo before, now not blocked. Occurs either if object is removed
// from  the document, or if user unblocks this object (e.g. by clicking on
// patch which was shown instead of object).
IPC_MESSAGE_ROUTED2(ViewHostMsg_BlockedTurboContentRemoved,
                    TurboContentSettingsType, /* type of blocked content */
                    std::string /* resource identifier */)
...
// Sent to the browser process when we wish to sample the scroll speed of the
// top view during a short time period. The information is sent back through a
// ViewHostMsg_SampledScrollSpeed message.
//
// This is currently only used to try to detect scroll changes due to fling
// gestures
IPC_MESSAGE_ROUTED0(ViewMsg_SampleScrollSpeed)

// Sent as a response to a ViewMsg_SampleScrollSpeed message after having
// sampled the scroll speed for a short time
//
// All data is in scaled contents coordinates
IPC_MESSAGE_ROUTED4(ViewHostMsg_SampledScrollSpeed,
                    gfx::Point, /* scroll delta */
                    gfx::Point, /* position before scroll */
                    gfx::Point, /* total size of the view */
                    int /* sampled timespan in ms */)

// Sent to the browser process to request a scroll animation to the top or
// bottom of the page
IPC_MESSAGE_ROUTED1(ViewMsg_ScrollToVerticalEdgeOfView,
                    bool /* scroll to top of the document */)
