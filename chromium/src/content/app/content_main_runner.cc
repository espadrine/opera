...
int RunZygote(const MainFunctionParams& main_function_params,
  ...
  MemoryAllowanceAgent::GetInstance()->SpecializeForProcess();
...
#if defined(OS_POSIX) && !defined(OS_MACOSX) && !defined(OS_ANDROID)
  // Processes forked from the Zygote do not run through here. They
  // are specialized in RunZyote() instead. The Zygote process itself
  // does not participate in the memory allowance logic.
...
class ContentMainRunnerImpl : public ContentMainRunner {
 public:
  ...
  static void SetMemoryCollarFunctionThunk(MemoryCollarFunction f) {
    MallocExtension::instance()->SetMemoryCollarFunction(f);
  }

  static bool MemoryCollar(int64 diff) {
    return MemoryAllowanceAgent::GetInstance()->
        ValidateMemoryChange(diff, false /* test */);
  }

  static bool V8MemoryCollar(int64_t diff, bool test) {
    VLOG_IF(2, !test) << "V8MemoryCollar: " << diff;
    VLOG_IF(3, test) << "V8MemoryCollar test: " << diff;
    return MemoryAllowanceAgent::GetInstance()->
        ValidateMemoryChange(diff, test);
  }
  ...
  virtual int Initialize(int argc,
    ...
    base::allocator::SetSetMemoryCollarFunctionFunction(
        SetMemoryCollarFunctionThunk);
...
#if !defined(OS_MACOSX) && defined(USE_TCMALLOC)
    if (delegate)
      delegate->MemoryAllowanceSetup();

    // GetInstance() ensures that the instances are allocated early.
    // Allocating these later might be racy and cause reentrancy problems.
    MemoryAllowanceController::GetInstance();

    if (MemoryAllowanceAgent::GetInstance()->Enabled()) {
      base::allocator::SetMemoryCollarFunction(MemoryCollar);
      v8::V8::SetMemoryCollarFunction(V8MemoryCollar);
    }
#endif
