...
class ChildProcessLauncher::Context
 ...
 public:
  ...
  void Launch(
    ...
    ipcfd_ = ipcfd;
#endif
    if (!MemoryAllowanceController::GetInstance()->Enabled()) {
      ...
      // When memory allowance is used the new process must first be
      // given clearance by the MemoryAllowanceController on the
      // IO-thread.  If there is enough memory allowance to launch the
      // process it will be given a startup allowance and launched on
      // the process launcher thread.
      BrowserThread::PostTask(
          BrowserThread::IO, FROM_HERE,
          base::Bind(
              &Context::MemoryAllowanceLaunch,
              make_scoped_refptr(this),
              client_thread_id_,
              child_process_id,
#if defined(OS_WIN)
 ...
 private:
  ...
  static void MemoryAllowanceLaunch(
      // |this_object| is NOT thread safe. Only use it to post a task back.
      scoped_refptr<Context> this_object,
      BrowserThread::ID client_thread_id,
      int child_process_id,
    ...
    int64 startup_allowance = MemoryAllowanceController::GetInstance()->
                                InitializeProcessAllowance(child_process_id);

    cmd_line->AppendSwitchASCII(switches::kMemoryAllowance,
                                base::Int64ToString(startup_allowance));

    if (!startup_allowance) {
