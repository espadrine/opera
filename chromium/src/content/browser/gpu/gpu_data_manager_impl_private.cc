...
void ApplyAndroidWorkarounds(const gpu::GPUInfo& gpu_info,
  ...
  bool is_mali = is_arm && renderer.find("mali") != std::string::npos;
  bool is_adreno = is_qualcomm && renderer.find("adreno") != std::string::npos;
  bool is_videocore = renderer.find("videocore") != std::string::npos;
  bool is_nvidia_l = vendor.find("nvidia") != std::string::npos;
  bool is_nvidia =
  ...
  // Mali: avoid minor context switching perf problems
  // QualComm, NVIDIA: Crashes with share groups
  if (is_vivante || is_img || is_nexus7 || is_qualcomm ||
      is_mali || is_adreno || is_videocore || is_nvidia || is_nvidia_l ||
      is_broadcom)
