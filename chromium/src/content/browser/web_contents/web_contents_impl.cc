...
bool WebContentsImpl::OnMessageReceived(RenderViewHost* render_view_host,
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(WebContentsImpl, message, message_is_ok)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_SampledScrollSpeed, OnSampledScrollSpeed)
...
void WebContentsImpl::SearchUsingText(const string16& text) {
  if (delegate_)
    delegate_->SearchUsingText(text);
}
...
void WebContentsImpl::OnSampledScrollSpeed(gfx::Point delta,
                                           gfx::Point start_position,
                                           gfx::Point view_size,
                                           int sampled_timespan) {
  if (GetRenderViewHostImpl() &&
      GetRenderViewHostImpl()->DidRecentlyProcessFling())
    delegate_->OnSampledScrollSpeed(delta, start_position, view_size,
                                    sampled_timespan);
}
...
void WebContentsImpl::DidNavigateMainFramePostCommit(
  ...
  // When navigating back from a ref URL to the top
  // non ref URL is_in_page is false even the navigation is
  // in fact in page (see AreURLsInPageNavigation()). In order
  // to not clear the displayed_insecure_content_ flag too eagerly
  // check navigation type as well.
...
RendererPreferences WebContentsImpl::GetRendererPrefs(
  ...
  if (delegate_)
    delegate_->OverrideRendererPreferences(&renderer_preferences_);
  return renderer_preferences_;
}
...
void WebContentsImpl::UpdatePlayState(WebKit::WebMediaPlayState play_state) {
  if (delegate_)
    delegate_->UpdatePlayState(this, play_state);
}
