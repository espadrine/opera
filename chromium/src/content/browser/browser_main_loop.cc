...
#if (defined(OS_LINUX) || defined(OS_OPENBSD)) && defined(USE_GLIB)
#include <glib-object.h>
#endif

#if defined(OS_LINUX) && !defined(LINUX_EMBEDDED)
...
#if defined(ENABLE_JSPLUGINS)
// static
base::WaitableEvent* BrowserMainLoop::GetJSPluginsThreadShutDownEvent() {
  DCHECK(g_current_browser_main_loop->jsplugins_thread_.get());
  return g_current_browser_main_loop->jsplugins_thread_.get()->
      GetShutDownEvent();
}
#endif  // defined(ENABLE_JSPLUGINS)
...
#endif
#if defined(ENABLE_JSPLUGINS)
    } else if (id == BrowserThread::JSPLUGINS) {
      jsplugins_thread_.reset(
            new BrowserProcessJSPluginsSubThread());
      jsplugins_thread_->StartWithOptions(*options);
#endif  // defined(ENABLE_JSPLUGINS)
