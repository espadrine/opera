...
bool SiteInstance::IsSameWebSite(BrowserContext* browser_context,
  ...
  // Use the same site instance when navigating to internal ui.
  if (url2.SchemeIs(chrome::kChromeInternalUIScheme))
    return true;

  // Use the same site instance when navigating from internal ui, but separate
  // from WebUI.
  if (url1.SchemeIs(chrome::kChromeInternalUIScheme))
    return !url2.SchemeIs(chrome::kChromeUIScheme);
