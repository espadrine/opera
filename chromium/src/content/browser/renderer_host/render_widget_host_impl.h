...
class CONTENT_EXPORT RenderWidgetHostImpl : virtual public RenderWidgetHost,
 ...
 public:
  ...
  virtual void SelectWord(const gfx::Point& position) OVERRIDE;
  ...
  virtual void RequestFrameCallback(
      const base::Callback<void()>& callback) OVERRIDE;
  virtual void RequestOffscreenFrameCallback(
      const base::Callback<void()>& callback) OVERRIDE;
  ...
  // Schedule a render widget compositor frame repaint.
  void Frame(bool force = false);

  void ActivateOutputSurface();
  ...
  // Swap the selection anchor and focus points.
  void SwapSelectionAnchorAndFocus();

  // Move the selection focus point.
  void MoveSelectionFocus(const gfx::Point&  point);
  ...
  // Checks whether a fling start event was processed recently. Used when
  // guessing if scroll events originate from fling gestures
  bool DidRecentlyProcessFling() { return fling_processed_recently_; }
  ...
  void RenderWidgetFrameReady(bool offscreen);
  void AcceleratedWidgetBuffersSwapped();
 ...
 private:
  ...
  void OnMsgMoveSelectionFocusAck();
  ...
  // Called with some delay after a gesture fling start event was processed.
  void OnRecentFlingGestureTimeout();
  ...
  // true if a ViewMsg_MoveSelectionFocus message was sent to the render view
  // and we're waiting for a ViewHostMsg_MoveSelectionFocus_ACK message.
  bool move_selection_focus_pending_;
  ...
  // Focus point for the next ViewMsg_MoveSelectionFocus message to send (only
  // non-null while |move_selection_focus_pending_| is true).
  scoped_ptr<gfx::Point> next_selection_focus_move_;
  ...
  // Keeps track of whether a fling start event was processed recently (within the
  // last kSampleScrollSpeedMsgTimeoutMs milliseconds).
  bool fling_processed_recently_;
  ...
  base::Callback<void()> frame_callback_;
  base::Callback<void()> offscreen_frame_callback_;
  ...
  // Remember if a widget should activate its output surface when it is bound.
  bool is_output_surface_bound_;
  bool pending_output_surface_active_;

  bool pending_frame_;
  bool pending_frame_forced_;
  bool pending_frame_ready_;
  bool pending_post_resize_frame_;
