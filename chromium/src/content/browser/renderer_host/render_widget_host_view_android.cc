...
RenderWidgetHostViewAndroid::RenderWidgetHostViewAndroid(
  ...
  }  // IsBrowserSideCompositorEnabled
  ...
  if (!IsBrowserSideCompositorEnabled()) {
    if (content_view_core) {
      physical_size_ = content_view_core_->GetPhysicalBackingSize();

      if (content_view_core->IsVisible())
        host_->ActivateOutputSurface();
    }
  }
}
...
bool RenderWidgetHostViewAndroid::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidgetHostViewAndroid, message)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_NonEmptyTreeActivated,
                        OnNonEmptyTreeActivated)
...
void RenderWidgetHostViewAndroid::WasResized(bool forced) {
  if (!forced && is_active_) {
    host_->WasResized();
    return;
  }
...
void RenderWidgetHostViewAndroid::SetPhysicalBackingSize(
    const gfx::Size& size) {
...
bool RenderWidgetHostViewAndroid::PopulateBitmapWithContents(
  ...
  WebKit::WebGLId texture = helper->CopyAndScaleTexture(
      texture_id_in_layer_, texture_size_in_layer_, size,
      true, GLHelper::SCALER_QUALITY_GOOD);
  ...
  helper->ReadbackTextureSync(
      texture, gfx::Rect(size),
      static_cast<unsigned char*>(skbitmap->getPixels()));
...
bool RenderWidgetHostViewAndroid::IsShowing() {
  ...
  if (IsBrowserSideCompositorEnabled())
    ...
    return is_visible_ && content_view_core_
        && content_view_core_->IsVisible();
}

bool RenderWidgetHostViewAndroid::IsActive() {
  DCHECK(!IsBrowserSideCompositorEnabled());
  return is_active_;
}

gfx::Size RenderWidgetHostViewAndroid::GetContentViewSize() const {
...
gfx::Rect RenderWidgetHostViewAndroid::GetViewBounds() const {
  ...
  if (!IsBrowserSideCompositorEnabled() && physical_size_.IsEmpty())
    return gfx::Rect();
  ...
  return gfx::Rect(GetContentViewSize());
}

gfx::Size RenderWidgetHostViewAndroid::GetPhysicalBackingSize() const {
  if (!content_view_core_ || content_view_core_->GetViewportSizeDip().IsEmpty())
    return gfx::Size();
...
void RenderWidgetHostViewAndroid::OnNonEmptyTreeActivated() {
  content_view_core_->GetWebContents()->GetDelegate()->NonEmptyTreeActivated();
}
...
void RenderWidgetHostViewAndroid::UpdateContentViewCoreFrameMetadata(
  ...
  if (content_view_core_) {
    ...
    content_view_core_->UpdateFrameInfo(
        ...
        frame_metadata.top_bar_animation);
  }
}
...
void RenderWidgetHostViewAndroid::BuffersSwapped(
  ...
  host_->RenderWidgetFrameReady(false);
}
...
void RenderWidgetHostViewAndroid::CompositorBuffersPresented() {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));

  std::vector<base::Callback<void()> >::iterator it;
  for (it = frame_callbacks_.begin(); it < frame_callbacks_.end(); it++)
    it->Run();
  frame_callbacks_.clear();
}

void RenderWidgetHostViewAndroid::RequestFrameCallback(
...
void RenderWidgetHostViewAndroid::ClearFocusedNode() {
  if (host_)
    host_->Send(new ViewMsg_ClearFocusedNode(host_->GetRoutingID()));
}

void RenderWidgetHostViewAndroid::FrameScheduled() {
  DCHECK(!IsBrowserSideCompositorEnabled());
...
void RenderWidgetHostViewAndroid::ResizeStarted(const gfx::Size& size) {
  DCHECK(!IsBrowserSideCompositorEnabled());
  pending_size_ = size;
  resize_started_ = true;

  if (content_view_core_)
    viewbounds_size_ = GetContentViewSize();
}

void RenderWidgetHostViewAndroid::SyncBufferSize(const gfx::Size& size) {
  DCHECK(!IsBrowserSideCompositorEnabled());
  bool resize_completed = false;

  if (resize_started_ && size == pending_size_) {
    resize_completed = true;
    resize_started_ = false;
  }

  if (resize_started_ && (size.IsEmpty() || size == physical_size_)) {
    host_->Frame(true);
  } else if (!size.IsEmpty()) {
    SetPhysicalBackingSize(size);
  }

  if (resize_completed && content_view_core_)
    content_view_core_->OnResizeComplete();
}

bool RenderWidgetHostViewAndroid::IsResizing() {
  return resize_started_;
}
...
void RenderWidgetHostViewAndroid::SwapSelectionAnchorAndFocus() {
  if (host_)
    host_->SwapSelectionAnchorAndFocus();
}

void RenderWidgetHostViewAndroid::MoveSelectionFocus(const gfx::Point& point) {
  if (host_)
    host_->MoveSelectionFocus(point);
}
...
void RenderWidgetHostViewAndroid::ScrollToVerticalEdge(bool scrollToTop) {
  if (host_)
    host_->Send(new ViewMsg_ScrollToVerticalEdgeOfView(host_->GetRoutingID(),
                                                       scrollToTop));
}
...
void RenderWidgetHostViewAndroid::Deactivate() {
  physical_size_ = gfx::Size(0, 0);
  is_active_ = false;
}
