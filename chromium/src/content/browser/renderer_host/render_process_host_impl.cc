...
#include "content/browser/external/media_player_filter_external.h"
#endif
...
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  channel_->AddFilter(MediaPlayerFilterExternal::Create());
#endif
  channel_->AddFilter(new HyphenatorMessageFilter(this));
  MemoryAllowanceController* mac = MemoryAllowanceController::GetInstance();
  if (mac->Enabled())
    channel_->AddFilter(mac->CreateMessageFilter(GetID()));
...
void RenderProcessHostImpl::PropagateBrowserCommandLineToRenderer(
  ...
  static const char* const kSwitchNames[] = {
    ...
    switches::kDisableBrowserSideCompositor,
...
#ifdef OPERA_TVSTORE
      IPC_MESSAGE_HANDLER(ViewHostMsg_LoadTVApp, OnLoadTVApp)
#endif
...
#ifdef OPERA_TVSTORE
void RenderProcessHostImpl::OnLoadTVApp(const ViewHostMsg_LoadTVApp_Params& params) {
  GetContentClient()->browser()->OnLoadTVApp(params);
}
#endif
