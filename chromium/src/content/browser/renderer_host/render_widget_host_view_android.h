...
class RenderWidgetHostViewAndroid
 ...
 public:
  ...
  virtual void CompositorBuffersPresented() OVERRIDE;
  virtual void RequestFrameCallback(base::Callback<void()> callback) OVERRIDE;
  ...
  void OnNonEmptyTreeActivated();

  int GetNativeImeAdapter();

  // Used only when browser side compositor is disabled.
  bool IsActive();
  void ResizeStarted(const gfx::Size& size);
  void SetPhysicalBackingSize(const gfx::Size& size) OVERRIDE;

  void WasResized();

  WebKit::WebGLId GetScaledContentTexture(float scale, gfx::Size* out_size);
  bool PopulateBitmapWithContents(jobject jbitmap);
  bool PopulateBitmapWithContents(SkBitmap* skbitmap) OVERRIDE;
  ...
  // Swap selection anchor and focus points.
  ...
  // Initiate an animation which will scroll to view either to the top or bottom
  void ScrollToVerticalEdge(bool scrollToTop);
 ...
 private:
  ...
  gfx::Size GetContentViewSize() const;

  // Deactivate the view. Being inactive means that after being invisible, it
  // has yet to have a frame scheduled. Only used when browser side compositor
  // is disabled.
  void Deactivate();
  ...
  // Frame callback list invoked after the content of this view has been
  // presented.
  std::vector<base::Callback<void()> > frame_callbacks_;
