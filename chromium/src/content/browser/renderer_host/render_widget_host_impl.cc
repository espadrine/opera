...
// The time to wait for a sample to be sent by the renderer
// after having issued a ViewMsg_SampleScrollSpeed
const int kSampleScrollSpeedMsgTimeoutMs = 350;
...
RenderWidgetHostImpl::RenderWidgetHostImpl(RenderWidgetHostDelegate* delegate,
    ...
    : view_(NULL),
      ...
      move_selection_focus_pending_(false),
      ...
      fling_processed_recently_(false),
      has_touch_handler_(false),
      weak_factory_(this),
      touch_event_queue_(new TouchEventQueue(this)),
      gesture_event_filter_(new GestureEventFilter(this)),
      is_output_surface_bound_(false),
      pending_output_surface_active_(false),
      pending_frame_(false),
      pending_frame_forced_(false),
      pending_frame_ready_(false),
      pending_post_resize_frame_(false),
...
void RenderWidgetHostImpl::RenderWidgetFrameReady(bool offscreen) {
  if (!frame_callback_.is_null() && !offscreen) {
    if (!IsBrowserSideCompositorEnabled()) {
      frame_callback_.Run();
      frame_callback_.Reset();
    } else {
      view_->RequestFrameCallback(frame_callback_);
      frame_callback_.Reset();
    }
  } else if (!offscreen_frame_callback_.is_null() && offscreen) {
    if (!IsBrowserSideCompositorEnabled()) {
      offscreen_frame_callback_.Run();
      offscreen_frame_callback_.Reset();
    }
  }
}

void RenderWidgetHostImpl::AcceleratedWidgetBuffersSwapped() {
  if (!IsBrowserSideCompositorEnabled()) {
    pending_frame_ready_ = false;
    if (pending_frame_) {
      Frame();
...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderWidgetHostImpl, msg, msg_is_ok)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_OutputSurfaceBound, OnMsgOutputSurfaceBound)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_MoveSelectionFocus_ACK,
                        OnMsgMoveSelectionFocusAck)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_Invalidate,
                        OnInvalidate)
...
void RenderWidgetHostImpl::Frame(bool force) {
  if (!view_) {
    return;
  }

  if (resize_ack_pending_) {
    pending_post_resize_frame_ = true;
    pending_frame_forced_ = pending_frame_forced_ || force;
  } else if (!pending_frame_ready_ &&
             !view_->GetPhysicalBackingSize().IsEmpty() &&
             ((force || pending_frame_forced_) ||
              !view_->IsResizing())) {
    ViewMsg_Frame_Params params;
    params.schedule = true;
    params.force = pending_frame_forced_ || force;
    Send(new ViewMsg_Frame(GetRoutingID(), params));

    view_->FrameScheduled();
    pending_frame_forced_ = false;
...
void RenderWidgetHostImpl::ActivateOutputSurface() {
  // If necessary, wait for the output surface to be bound to the view. If the
  // message is sent before bound, it won't be received by the output surface.
  if (!is_output_surface_bound_)
    pending_output_surface_active_ = true;
  else
    Send(new ViewMsg_ActivateOutputSurface(GetRoutingID()));
}
...
void RenderWidgetHostImpl::ForwardGestureEvent(
  ...
  if (gesture_event.type == WebKit::WebGestureEvent::GestureFlingStart) {
    Send(new ViewMsg_SampleScrollSpeed(GetRoutingID()));
  }
...
void RenderWidgetHostImpl::RequestFrameCallback(
    const base::Callback<void()>& callback) {
  frame_callback_ = callback;
}

void RenderWidgetHostImpl::RequestOffscreenFrameCallback(
    const base::Callback<void()>& callback) {
  offscreen_frame_callback_ = callback;
}
...
void RenderWidgetHostImpl::RendererExited(base::TerminationStatus status,
  ...
  next_selection_focus_move_.reset();
  ...
  // Reset output surface state
  is_output_surface_bound_ = false;

  // Reset frame scheduler state
  pending_frame_ready_ = false;
  pending_frame_ = false;
  pending_frame_forced_ = false;
...
void RenderWidgetHostImpl::OnMsgOutputSurfaceBound() {
  is_output_surface_bound_ = true;

  if (pending_output_surface_active_) {
    pending_output_surface_active_ = false;
    Send(new ViewMsg_ActivateOutputSurface(GetRoutingID()));
  }
}
...
void RenderWidgetHostImpl::OnUpdateRect(
  ...
  if (is_resize_ack && !IsBrowserSideCompositorEnabled()) {
    // We got a frame request during resize, so invoke it now, if possible.
    if (pending_post_resize_frame_) {
      pending_post_resize_frame_ = false;
      Frame();
    }
  }
...
void RenderWidgetHostImpl::OnRecentFlingGestureTimeout() {
  fling_processed_recently_ = false;
}
...
void RenderWidgetHostImpl::OnMsgMoveSelectionFocusAck() {
  move_selection_focus_pending_ = false;
  if (next_selection_focus_move_.get()) {
    scoped_ptr<gfx::Point> next(next_selection_focus_move_.Pass());
    MoveSelectionFocus(*next);
  }
}
...
void RenderWidgetHostImpl::ProcessGestureAck(bool processed, int type) {
  ...
  if (type == WebGestureEvent::GestureFlingStart && processed &&
      !fling_processed_recently_) {
    fling_processed_recently_ = true;
    base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
        base::Bind(&RenderWidgetHostImpl::OnRecentFlingGestureTimeout,
                   weak_factory_.GetWeakPtr()),
        base::TimeDelta::FromMilliseconds(kSampleScrollSpeedMsgTimeoutMs));
  }
}
...
void RenderWidgetHostImpl::OnInvalidate() {
  DCHECK(!IsBrowserSideCompositorEnabled());
  Frame();
}
...
void RenderWidgetHostImpl::SwapSelectionAnchorAndFocus() {
  Send(new ViewMsg_SwapSelectionAnchorAndFocus(GetRoutingID()));
}

void RenderWidgetHostImpl::MoveSelectionFocus(const gfx::Point& point) {
  if (move_selection_focus_pending_) {
    if (!next_selection_focus_move_.get()) {
      next_selection_focus_move_.reset(new gfx::Point());
    }
    *next_selection_focus_move_ = point;
    return;
  }

  move_selection_focus_pending_ = true;
  Send(new ViewMsg_MoveSelectionFocus(GetRoutingID(), point));
}
...
void RenderWidgetHostImpl::SelectWord(const gfx::Point& position) {
  Send(new ViewMsg_SelectWord(GetRoutingID(), position));
  RecordAction(UserMetricsAction("SelectWord"));
}
