...
#include "content/browser/external/media_player_manager_external.h"
#endif
...
using WebKit::WebMediaPlayState;
...
#endif
#if defined(OPERA_TVSDK) && defined(ENABLE_UVA)
  media_player_manager_ = MediaPlayerManagerExternal::Create(this);
#endif
}
...
void RenderViewHostImpl::OnShowSelectionPreview(
    ...
    const TransportDIB::Id& bitmap_id,
    const gfx::Rect& focus_rect) {
  DCHECK(!bitmap_size.IsEmpty());
  TransportDIB* dib = GetProcess()->GetTransportDIB(bitmap_id);
  DCHECK(dib->memory());

  if (dib->size() < SkBitmap::ComputeSize(SkBitmap::kARGB_8888_Config,
                                          bitmap_size.width(),
                                          bitmap_size.height())) {
    return;
  }
  ...
  preview_bitmap.setConfig(SkBitmap::kARGB_8888_Config,
                           bitmap_size.width(), bitmap_size.height());
  preview_bitmap.setPixels(dib->memory());

  if (view_)
    view_->ShowSelectionPreview(preview_bitmap, focus_rect);
...
void RenderViewHostImpl::SuspendMedia() {
  Send(new ViewMsg_SuspendMedia(GetRoutingID()));
}

void RenderViewHostImpl::ResumeSuspendedMedia() {
  Send(new ViewMsg_ResumeSuspendedMedia(GetRoutingID()));
}
...
void RenderViewHostImpl::RequestBitmapResult(int32 request_id,
                                             const SkBitmap& bitmap) {
  std::map<int32, base::Callback<void(const SkBitmap&)> >::iterator it;
  it = bitmap_requests_.find(request_id);
  it->second.Run(bitmap);
  bitmap_requests_.erase(it);
}

void RenderViewHostImpl::RequestBitmap(
    const gfx::Size& size, bool wait_for_frame,
    base::Callback<void(const SkBitmap&)> callback) {
  int32 request_id = next_bitmap_request_id_++;
  bitmap_requests_[request_id] = callback;
  Send(new ViewMsg_RequestBitmap(
      GetRoutingID(), request_id, size, wait_for_frame));
}
...
void RenderViewHostImpl::OnUpdatePlayState(WebMediaPlayState play_state) {
