...
void DevToolsHttpHandlerImpl::AddIgnoreScheme(const std::string& scheme) {
  ignore_schemes_.insert(scheme);
}

void DevToolsHttpHandlerImpl::RemoveIgnoreScheme(const std::string& scheme) {
  ignore_schemes_.erase(scheme);
}
...
bool DevToolsHttpHandlerImpl::IsFilteredScheme(RenderViewHost* rvh) {
  std::string scheme = rvh->
      GetDelegate()->GetAsWebContents()->GetURL().scheme();
  return ignore_schemes_.count(scheme) != 0;
}
