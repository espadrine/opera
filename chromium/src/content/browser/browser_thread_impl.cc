...
static const char* g_browser_thread_names[BrowserThread::ID_COUNT] = {
  ...
  "Chrome_JSPluginsThread",  // JSPLUGINS
...
#if defined(ENABLE_JSPLUGINS)
NOINLINE void BrowserThreadImpl::JSPluginsThreadRun(base::MessageLoop* message_loop) {
  volatile int line_number = __LINE__;
  Thread::Run(message_loop);
  CHECK_GT(line_number, 0);
}
...
void BrowserThreadImpl::Run(base::MessageLoop* message_loop) {
  ...
  switch (thread_id) {
    ...
    case BrowserThread::JSPLUGINS:
      return JSPluginsThreadRun(message_loop);
#endif  // defined(ENABLE_JSPLUGINS)
