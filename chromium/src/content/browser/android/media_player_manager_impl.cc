...
void MediaPlayerManagerImpl::ExitFullscreen(bool release_media_player) {
  ...
  if (!player)
    return;
  if (release_media_player || !media::IsSurfaceTextureSupported()) {
    player->Release();
    Send(new MediaPlayerMsg_DidMediaPlayerPause(
...
void MediaPlayerManagerImpl::SetFullscreenSurfaceHolder(
    jobject surface_holder) {
  MediaPlayerAndroid* player = GetFullscreenPlayer();
  if (player) {
    player->SetVideoSurfaceHolder(surface_holder);
