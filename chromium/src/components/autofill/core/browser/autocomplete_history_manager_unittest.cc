...
TEST_F(AutocompleteHistoryManagerTest, SSNValue) {
  ...
  EXPECT_CALL(*web_data_service_.get(), AddFormFields(_)).Times(0);
  autocomplete_manager_->OnFormSubmitted(form);
  autocomplete_manager_->OnFormSubmitted(FormStructure(form, ""));
}
...
TEST_F(AutocompleteHistoryManagerTest, SearchField) {
  ...
  EXPECT_CALL(*(web_data_service_.get()), AddFormFields(_)).Times(1);
  search_field.value = ASCIIToUTF16("my another favorite query");
  autocomplete_manager_->OnFormSubmitted(FormStructure(form, ""));
}

// Any field which type is credit card data related should be rejected.
TEST_F(AutocompleteHistoryManagerTest,
       OnFormSubmitted_FormStructure_CreditCardField_Rejected) {
  FormData form;
  form.name = ASCIIToUTF16("MyForm");
  form.method = ASCIIToUTF16("POST");
  form.origin = GURL("http://myform.com/form.html");
  ...
  FormFieldData valid_cc;
  valid_cc.label = ASCIIToUTF16("AnyLabel");
  valid_cc.name = ASCIIToUTF16("AnyName");
  valid_cc.value = ASCIIToUTF16("AnyValue");
  valid_cc.form_control_type = "text";
  form.fields.push_back(valid_cc);

  FormStructure form_structure(form, "");
  EXPECT_CALL(*web_data_service_.get(), AddFormFields(_)).Times(0);
  for (int i = CREDIT_CARD_NAME; i <= CREDIT_CARD_VERIFICATION_CODE; i++) {
    form_structure.field(0)->set_heuristic_type(
        static_cast<AutofillFieldType>(i));
    autocomplete_manager_->OnFormSubmitted(form_structure);
  }
}
