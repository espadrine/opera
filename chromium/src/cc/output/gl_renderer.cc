...
void GLRenderer::ClearTop(gfx::Size viewport_size, int border) {
  GLC(context_,
      context_->viewport(0, viewport_size.height(), viewport_size.width(),
                         border));
  // TODO(peterp): This should be the page's background color.
  // The problem is that it's always black...
  GLC(context_, context_->clearColor(1, 1, 1, 1));
