...
class CC_EXPORT LayerTreeHost : NON_EXPORTED_BASE(public RateLimiterClient) {
 public:
  ...
  void DidChangeTopControlsAnimationState(bool active);
  ...
  void StartFrameLoopOnImplThread();
  void StopFrameLoopOnImplThread();

  void InvalidateOnImplThread();
  void Invalidate();
  void ForceFrameOnImplThread();
  void ScheduleFrameOnImplThread();
  void ScheduleFrame();
  ...
  void DidPinchGesture(float scale, const gfx::Point&);
  void MaxScrollUpdated(const gfx::Vector2d& maxScroll);
  void PendingTreeActivated(const gfx::Vector2d& scrollTotal);
 ...
 private:
  ...
  ScopedVector<PendingPageScaleAnimation> pending_page_scale_animations_;
  ...
  // true if the layer tree is being scaled, false otherwise.
  bool scaling_;
