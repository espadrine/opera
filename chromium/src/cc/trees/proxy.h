...
class CC_EXPORT Proxy {
 public:
  ...
  // Notify about that the back buffer size has changed. Only relevant when
  // dont_draw_when_resizing setting is enabled.
  virtual void ResizeStarted() = 0;

  // Called when a frame should been scheduled. Only relevant when frame driven
  // animation is enabled .
  virtual void ScheduleFrame() = 0;
  virtual void ScheduleFrameOnImplThread() = 0;

  // Force a frame, even when there is no damage. Only used when frame driven
  // animation is enabled.
  virtual void ForceFrameOnImplThread() = 0;
