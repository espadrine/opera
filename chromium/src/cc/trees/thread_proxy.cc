...
base::TimeTicks ThreadProxy::GetAnimationMonotonicTime()
{
...
void ThreadProxy::InvalidateOnImplThread() {
  layer_tree_host_->InvalidateOnImplThread();
}

void ThreadProxy::StartFrameLoopOnImplThread() {
  layer_tree_host_->StartFrameLoopOnImplThread();
}

void ThreadProxy::StopFrameLoopOnImplThread() {
  layer_tree_host_->StopFrameLoopOnImplThread();
}
...
ThreadProxy::ScheduledActionDrawAndSwapInternal(bool forced_draw) {
  ...
  if (layer_tree_host_impl_->CanDraw() &&
      layer_tree_host_impl_->CanRender() &&
...
void ThreadProxy::ResizeStarted() {
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHostImpl::ResizeStarted,
                 base::Unretained(layer_tree_host_impl_.get())));
}

void ThreadProxy::ForceFrameOnImplThread() {
  layer_tree_host_impl_->ForceFrame();
}

void ThreadProxy::ScheduleFrameOnImplThread() {
  layer_tree_host_impl_->ScheduleFrame();
}

void ThreadProxy::ScheduleFrame() {
  DCHECK(IsMainThread());
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHostImpl::ScheduleFrame,
                 base::Unretained(layer_tree_host_impl_.get())));
}
...
void ThreadProxy::NonEmptyTreeActivated() {
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHost::NonEmptyTreeActivated,
...
void ThreadProxy::DidPinchGesture(float scale, const gfx::Point& anchor)
{
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHost::DidPinchGesture,
                 base::Unretained(layer_tree_host_),
                 scale,
                 anchor));
}

void ThreadProxy::MaxScrollUpdated(const gfx::Vector2d& max_scroll)
{
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHost::MaxScrollUpdated,
                 base::Unretained(layer_tree_host_),
                 max_scroll));
}

void ThreadProxy::PendingTreeActivated(const gfx::Vector2d& scrollTotal)
{
  Proxy::ImplThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&LayerTreeHost::PendingTreeActivated,
                 base::Unretained(layer_tree_host_),
                 scrollTotal));
}
...
void ThreadProxy::DidChangeTopControlsAnimationState(bool active) {
  layer_tree_host_->DidChangeTopControlsAnimationState(active);
}
