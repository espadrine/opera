...
LayerTreeHostImpl::LayerTreeHostImpl(
    ...
    : client_(client),
      ...
      settings_(settings),
      visible_(true),
      first_nonempty_tree_activated_(false),
...
bool LayerTreeHostImpl::CanRender() {
  if (settings_.dont_draw_when_resizing) {
    if (resize_started_) {
      TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw resize started",
                           TRACE_EVENT_SCOPE_THREAD);
      return false;
    }
  }
  if (settings_.frame_driven_animation_enabled) {
    if (!frame_scheduled_) {
      TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw frame not scheduled",
                           TRACE_EVENT_SCOPE_THREAD);
      return false;
    }
  }
...
void LayerTreeHostImpl::StartPageScaleAnimation(gfx::Vector2d target_offset,
  ...
  // Do one initial step, thereby allowing startPageScaleAnimation to apply
  // instant scalings/scrolls in case the duration == 0.
  AnimatePageScale(base::TimeTicks::Now());
}

void LayerTreeHostImpl::ScheduleAnimation() {
  client_->SetNeedsRedrawOnImplThread();
  InvalidateOnImplThread();
}

base::TimeTicks LayerTreeHostImpl::GetAnimationMonotonicTime()
{
    return client_->GetAnimationMonotonicTime();
}
...
int LayerTreeHostImpl::TopBorder() const {
  if (settings().top_controls_adjust_viewport && top_controls_manager_)
    return top_controls_manager_->content_top_offset();
...
void LayerTreeHostImpl::ResizeStarted() {
  resize_started_ = true;
}

void LayerTreeHostImpl::ForceFrame() {
  frame_forced_ = true;
  client_->SetNeedsRedrawOnImplThread();
}

void LayerTreeHostImpl::ScheduleFrame() {
  frame_pending_ = false;
...
CompositorFrameMetadata LayerTreeHostImpl::MakeCompositorFrameMetadata() const {
  ...
  if (top_controls_manager_) {
    ...
    metadata.top_bar_animation =
        top_controls_manager_->animation_direction() !=
        TopControlsManager::NO_ANIMATION;
  }
...
void LayerTreeHostImpl::DidChangeTopControlsAnimationState(bool active) {
...
void LayerTreeHostImpl::MaxScrollUpdated(const gfx::Vector2d& max_scroll)
{
  client_->MaxScrollUpdated(max_scroll);
}
...
void LayerTreeHostImpl::ActivatePendingTree() {
  ...
  gfx::Vector2d scroll_total = gfx::ToFlooredVector2d(
      RootScrollLayer()->scroll_offset() + RootScrollLayer()->ScrollDelta());

  client_->PendingTreeActivated(scroll_total);
  ...
  if (!first_nonempty_tree_activated_ &&
      !device_viewport_size_.IsEmpty() &&
      active_tree_->root_layer()) {
    first_nonempty_tree_activated_ = true;
...
void LayerTreeHostImpl::SetVisible(bool visible) {
  ...
  UpdateFrameLoopMask(0);

  // When changing visibility we might end up in a situation when an
  // invalidation request didn't result in a frame event. To avoid this
  // reset the invalidate message throttler when visibility changes.
  frame_pending_ = false;
...
void LayerTreeHostImpl::ScrollEnd() {
  ...
  UpdateFrameLoopMask(frame_loop_mask_ & ~FRAME_LOOP_SCROLL);
}
...
void LayerTreeHostImpl::PinchGestureBegin() {
  ...
  client_->RenewTreePriority();
  UpdateFrameLoopMask(frame_loop_mask_ | FRAME_LOOP_PINCH);
}
...
void LayerTreeHostImpl::PinchGestureEnd() {
  pinch_gesture_active_ = false;
  client_->DidPinchGesture(active_tree_->page_scale_factor() * active_tree_->page_scale_delta(), previous_pinch_anchor_);
  client_->SetNeedsCommitOnImplThread();
  UpdateFrameLoopMask(frame_loop_mask_ & ~FRAME_LOOP_PINCH);
}
...
void LayerTreeHostImpl::InvalidateOnImplThread() {
  if (!settings_.frame_driven_animation_enabled)
    return;
...
void LayerTreeHostImpl::DidBeginScroll() {
  UpdateFrameLoopMask(frame_loop_mask_ | FRAME_LOOP_SCROLL);
}
...
void LayerTreeHostImpl::UpdateFrameLoopMask(uint32 mask) {
  if (!frame_loop_mask_ && mask)
    client_->StartFrameLoopOnImplThread();
  else if (!mask && frame_loop_mask_)
    client_->StopFrameLoopOnImplThread();

  frame_loop_mask_ = mask;
}
