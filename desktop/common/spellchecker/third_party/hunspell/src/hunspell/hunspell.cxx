...
#endif
{
...
#elif defined HUNSPELL_OPERA_CLIENT
    std::stringstream aff_data;
    std::stringstream dic_data;
        ...
        while (reader.HasMore())
        {
          if (reader.OpenCurrentEntryInZip())
          {
            ...
            if (reader.current_entry_info()->file_path().MatchesExtension(
              ...
              if (reader.ExtractCurrentEntryIntoStream(&aff_data))
                init_mask |= HAS_AFF;
            } else if (reader.current_entry_info()->file_path().MatchesExtension(
                       FILE_PATH_LITERAL(".dic"))) {
              if (reader.ExtractCurrentEntryIntoStream(&dic_data))
                init_mask |= HAS_DICT;
            }
          }
    ...
    {
      aff_data.clear();
      aff_data.str(std::string());
      dic_data.clear();
      dic_data.str(std::string());
    }
    pHMgr[0] = new HashMgr(aff_data.str(), dic_data.str());
    if (pHMgr[0]) maxdic = 1;

    pAMgr = new AffixMgr(aff_data.str(), pHMgr, &maxdic);
#else
